#!/bin/bash
# Local native-PyTorch inference + NeMo-Skills benchmark/scoring pipeline.
...
SERVER_SCRIPT="$ROOT_DIR/pytorch_nemo_eval/pytorch_openai_server.py"
MERGE_SCRIPT="$ROOT_DIR/pytorch_nemo_eval/add_pytorch_metrics_to_metrics.py"
...
PYTORCH_PORT="${PYTORCH_PORT:-32000}"
PYTORCH_DTYPE="${PYTORCH_DTYPE:-bfloat16}"
PYTORCH_BLOCK_LENGTH="${PYTORCH_BLOCK_LENGTH:-8}"
PYTORCH_THRESHOLD="${PYTORCH_THRESHOLD-}"
PYTORCH_CAUSAL_CONTEXT="${PYTORCH_CAUSAL_CONTEXT:-true}"
...
if ! "$PYTORCH_PYTHON" -c "import torch, transformers, fastapi, uvicorn" >/dev/null 2>&1; then
    echo "ERROR: PYTORCH_PYTHON cannot import torch/transformers/FastAPI/Uvicorn: $PYTORCH_PYTHON" >&2
    exit 1
fi
...
echo "=============================================================="
echo " Native PyTorch + NeMo-Skills evaluation"
echo "=============================================================="
...
[[ -n "$PYTORCH_THRESHOLD" ]] && SERVER_ARGS+=(--threshold "$PYTORCH_THRESHOLD")
[[ "$PYTORCH_CAUSAL_CONTEXT" == "false" ]] && SERVER_ARGS+=(--no-causal-context)
...
echo "[5/5] Completed native PyTorch evaluation pipeline."
