#!/bin/bash
# LinearSpec confidence/rank diagnostic experiment.
...
SUMMARY_SCRIPT="$PROJECT_DIR/xp/sglang_eval/summarize_linearspec_confidence_trace.py"

DEFAULT_OUTPUT_PATH="$OBSERVATION_RESULTS_ROOT/sglang_linearspec_confidence_results"
...
Purpose:
  Run LinearSpec confidence/rank diagnostics for one or more NeMo-Skills benchmarks.
  Each benchmark is evaluated in a separate SGLang run so raw trace files do not mix.
...
SGLang / eval controls:
  ...
  --include-values            Also store raw value arrays in summary JSON
  --no-values                 Legacy alias; raw values are already disabled by default
...
settings = {
    ...
    "entrypoint": "observations/eval_linearspec_confidence.sh",
    "argv": sys.argv[2:],
    "command": "bash observations/eval_linearspec_confidence.sh " + " ".join(shlex.quote(x) for x in sys.argv[2:]),
    ...
    "include_values_in_summary": os.environ["INCLUDE_VALUES"],
}
...
for bench_spec in "${BENCH_ARRAY[@]}"; do
    ...
    trace_file="$TRACE_DIR/raw_trace_${bench_name}.jsonl"
    summary_file="$SUMMARY_DIR/confidence_distribution_${bench_name}.json"
    ...
    status=0
    SGLANG_CONFIDENCE_TRACE_FILE="$trace_file" "${cmd[@]}" || status=$?
    if [[ "$status" != "0" ]]; then
        ANY_FAILURE=1
        echo "WARNING: benchmark $bench_spec exited with status $status; summarizing trace if present." >&2
    fi

    summary_cmd=("$SGLANG_PYTHON" "$SUMMARY_SCRIPT" --trace-file "$trace_file" --output-json "$summary_file" --benchmark "$bench_name" --benchmark-spec "$bench_spec")
    [[ "$INCLUDE_VALUES" == "true" ]] && summary_cmd+=(--include-values)
    "${summary_cmd[@]}"
...
echo ""
echo "Completed LinearSpec confidence/rank diagnostic eval."
echo "Output dir: $FINAL_DIR"
