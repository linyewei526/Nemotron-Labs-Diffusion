#!/usr/bin/env python3
"""Read-only GPU selection and free-port discovery for the observation entry."""
...
def select_gpu(candidates: str, minimum_free_gb: float) -> int:
    ...
    minimum_mib = float(minimum_free_gb) * 1024
    eligible = [row for row in rows if row["free_mib"] >= minimum_mib]
    if not eligible:
        detail = ", ".join(
        ...
        raise RuntimeError(
            f"no candidate GPU has >= {minimum_free_gb:g} GiB free; {detail}"
        )
    # Serving observation first needs enough memory, then follows the user's
    # preference for lower compute occupancy; ties prefer more free memory.
    eligible.sort(
        key=lambda row: (
            row["gpu_util"],
            row["memory_util"],
...
def main() -> int:
    ...
    port.add_argument("--start", type=int, default=34000)
    check = subparsers.add_parser("check-port")
