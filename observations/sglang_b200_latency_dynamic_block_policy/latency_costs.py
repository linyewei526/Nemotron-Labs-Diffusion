#!/usr/bin/env python3
"""Parse and freeze the B8/B16/B32 two-forward latency table from the report."""
...
def cold_start_block(concurrency: int) -> int:
    """Return the protocol's first-round block for a nominal concurrency."""
    try:
        return COLD_START_BLOCKS[int(concurrency)]
    except KeyError as exc:
        raise ValueError(
            f"unsupported nominal concurrency C={concurrency}; "
            f"expected one of {list(DEFAULT_CONCURRENCIES)}"
        ) from exc


def parse_cost_document(path: Path) -> Dict[int, Dict[int, float]]:
    """Return costs[concurrency][block] in ms, rejecting incomplete tables."""
    raw = path.read_bytes()
    ...
    for line_number, line in enumerate(text.splitlines(), 1):
        ...
        if concurrency == 0 or fields[-1] == "N/A":
            continue
        try:
            value = float(fields[-1])
        except ValueError as exc:
            raise RuntimeError(f"{path}:{line_number}: invalid latency row") from exc
        old = by_block[block].get(concurrency)
        if old is not None and abs(old - value) > 1e-9:
            raise RuntimeError(
                f"{path}:{line_number}: conflicting B{block}/C{concurrency} costs"
            )
    ...
    if any(value <= 0 for row in costs.values() for value in row.values()):
        raise RuntimeError("all latency costs must be positive")
    return costs
...
def snapshot(
    ...
    for concurrency in concurrencies:
        if concurrency not in costs:
            raise RuntimeError(f"latency table has no C={concurrency}")
        selected[str(concurrency)] = {
    ...
    return {
        ...
        "units": "ms per one draft plus one causal verify Graph replay",
        "blocks": list(BLOCKS),
        ...
        "scope": "pure B200 forward only; no prefill, scheduling, bucketing, or wall time",
    }
...
def parse_int_list(text: str) -> list[int]:
    ...
    if not values or values[0] < 1 or values[-1] > 128:
        raise ValueError("concurrencies must be nonempty integers in [1,128]")
    return values
