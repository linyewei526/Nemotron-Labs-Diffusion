#!/bin/bash
# LinearSpec low-confidence token rejection diagnostic experiment.
...
SUMMARY_SCRIPT="$PROJECT_DIR/xp/sglang_eval/summarize_linearspec_low_confidence_rejection.py"

DEFAULT_OUTPUT_PATH="$OBSERVATION_RESULTS_ROOT/sglang_linearspec_low_confidence_results"
...
Purpose:
  Run LinearSpec low-confidence rejection diagnostics for one or more NeMo-Skills benchmarks.
  The experiment records every draft candidate confidence in each round and summarizes
  token_x_drop_abs / token_y_drop_pct accepted-vs-rejected counts.
...
Thresholds:
  --abs-start V               drop_abs start, inclusive (default: 0.300)
  --abs-end V                 drop_abs end, inclusive (default: 0.400)
  --abs-step V                drop_abs step (default: 0.005)
  --pct-start V               drop_pct start, inclusive (default: 0.40)
  --pct-end V                 drop_pct end, inclusive (default: 0.60)
...
RUN_NAME="linearspec_low_confidence_$(date +%Y%m%d_%H%M%S)"
...
settings = {
    ...
    "entrypoint": "observations/eval_linearspec_low_confidence_rejection.sh",
    "argv": sys.argv[2:],
    "command": "bash observations/eval_linearspec_low_confidence_rejection.sh " + " ".join(shlex.quote(x) for x in sys.argv[2:]),
    ...
    "thresholds": {
        "abs_start": os.environ["ABS_START"],
        "abs_end": os.environ["ABS_END"],
        "abs_step": os.environ["ABS_STEP"],
        "pct_start": os.environ["PCT_START"],
        "pct_end": os.environ["PCT_END"],
        "pct_step": os.environ["PCT_STEP"],
    },
}
...
for bench_spec in "${BENCH_ARRAY[@]}"; do
    ...
    trace_file="$TRACE_DIR/raw_low_confidence_trace_${bench_name}.jsonl"
    summary_file="$SUMMARY_DIR/low_confidence_rejection_${bench_name}.json"
    ...
    status=0
    SGLANG_CONFIDENCE_TRACE_FILE="" SGLANG_LOW_CONFIDENCE_TRACE_FILE="$trace_file" "${cmd[@]}" || status=$?
    if [[ "$status" != "0" ]]; then
        ANY_FAILURE=1
...
echo ""
echo "Completed LinearSpec low-confidence rejection diagnostic eval."
echo "Output dir: $FINAL_DIR"
