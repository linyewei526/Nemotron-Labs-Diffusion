# NLD PyTorch + NeMo-Skills：固定 margin-risk 多候选与 continuation overlap 实验

> 实验入口：`method/margin_risk_multi_overlap_linearspec/eval_margin_risk_multi_overlap.sh`  
> 独立代码：`method/margin_risk_multi_overlap_linearspec/`  
> 默认结果根目录：`/data/home/wly/dLLM/NLD_results/margin_risk_multi_overlap_results/`  
...
本方法是固定 margin-risk 单候选 overlap 的独立扩展，不修改下列任何既有实现：
...
└── margin_risk_multi_overlap_YYYYMMDD_HHMMSS/
    ├── Settings.json
    ├── report.md
    ...
    └── artifacts/<dataset>/
        ...
        ├── pytorch_margin_risk_multi_overlap_metrics_summary.json
...
等于 0.5 不触发。扫描会继续到 block 末尾，因此除保存 P1/P2/P3 外，也能准确知道整块 crossing 总数是 0、1、2 还是 3 及以上。

示例：某位置最高和第二高概率为 0.65、0.25，则 margin=0.40、margin-risk=0.60，会触发；0.75、0.15 对应 margin-risk=0.40，不触发。

### 2.2 候选修正分支

对 P1、P2、P3 各自选择替代 token B：排除该位置原 draft token A、MASK 和 EOS 后，取 logit 最大的 token。B 因排除了 EOS，不一定等同于未经约束的 raw top2。
...
若当前整块严格 crossing 总数不超过 2（包括 0），除候选分支外再追加一条 continuation row：
...
continuation 的触发依据是原始 crossing 总数，而不是经过 token/context/thinking 边界过滤后剩下的候选分支数。若 crossing 总数达到 3，则三个 row 名额全部用于 P1/P2/P3，不追加 new。
...
设 q 为 verifier 从左向右第一个不通过的 draft 位置；整块通过时 q 不存在。每次实际 fused 轮只属于以下 13 个状态之一：
...
|`miss_after_last`|q 在最后预测位置之后，所有预测位置原 token 均正确|P3=8，q=11|
...
|`candidate_3_fixed`|q=P3 且 P3 的 B 正确|第三个预测命中|
|`candidate_3_wrong`|q=P3 但 P3 的 B 错误|第三个预测未修正|
|`full_continuation_hit`|整块通过且 bonus=new[0]|new 可验证复用|
|`full_continuation_miss`|整块通过但 bonus≠new[0]|new 不可复用|
|`full_continuation_absent`|整块通过但本轮没有 new|crossing≥3 等情况|

P1/P2/P3 指原始从左向右 crossing rank。即使 P1 因预算边界被跳过、P2 仍可执行，P2 命中仍归入 `candidate_2_fixed`，不会重编号为 P1。

状态次数之和必须等于实际 fused 轮数 `prefetch_attempts`，字段 `outcome_partition_valid` 自动校验。报告另给出：

- 预测未命中实际错误位置的总次数和比例；
- P1/P2/P3 修正正确、错误的独立次数与比例；
- 整块通过且 new 命中；
- 整块通过但 new 未命中或不存在；
...
```bash
bash method/margin_risk_multi_overlap_linearspec/eval_margin_risk_multi_overlap.sh --help
```
...
```bash
bash method/margin_risk_multi_overlap_linearspec/eval_margin_risk_multi_overlap.sh --mode overlap_lora --benchmarks gsm8k:1 --max-samples 1 --tokens 64 --block-size 16 --threshold 0 --margin-risk-threshold 0.5 --temperature 0 --gpu-device 0 --gpu-memory-reserve-gb 0 --dry-run
```

### 4.3 一题全链路 smoke

```bash
bash method/margin_risk_multi_overlap_linearspec/eval_margin_risk_multi_overlap.sh --mode overlap_lora --benchmarks gsm8k:1 --max-samples 1 --tokens 64 --context-length 2112 --block-size 16 --threshold 0 --margin-risk-threshold 0.5 --temperature 0 --top-p 0.95 --disable-thinking --client-concurrency 1 --num-chunks 1 --gpu-device auto --gpu-min-free-gb 24 --gpu-memory-reserve-gb 0 --efficiency-only --keep-runtime
```
...
```bash
bash method/margin_risk_multi_overlap_linearspec/eval_margin_risk_multi_overlap.sh --mode overlap_lora --benchmarks gsm8k:1,human-eval:1,mbpp:1,math-500:1,aime24:1,aime25:1,gpqa:1,ifeval:1,livecodebench-cpp:1,mmlu:1 --tokens 8192 --context-length 10240 --block-size 16 --threshold 0 --margin-risk-threshold 0.5 --temperature 0 --top-p 0.95 --disable-thinking --client-concurrency 1 --num-chunks 1 --gpu-device auto --gpu-min-free-gb 24 --output-path /data/home/wly/dLLM/NLD_results/margin_risk_multi_overlap_results
```

### 4.6 单数据集和自定义多数据集

```bash
bash method/margin_risk_multi_overlap_linearspec/eval_margin_risk_multi_overlap.sh --mode overlap_lora --benchmarks gsm8k:1 --tokens 8192 --block-size 16 --margin-risk-threshold 0.5 --gpu-device 2
```

```bash
bash method/margin_risk_multi_overlap_linearspec/eval_margin_risk_multi_overlap.sh --mode overlap_lora --benchmarks gsm8k:1,math-500:1,aime25:1 --tokens 8192 --block-size 16 --margin-risk-threshold 0.5 --gpu-device 2
```
...
```bash
bash method/margin_risk_multi_overlap_linearspec/eval_margin_risk_multi_overlap.sh --mode overlap_lora --benchmarks gsm8k:1 --tokens 8192 --block-size 16 --margin-risk-threshold 0.5 --gpu-device 3
```

```bash
bash method/margin_risk_multi_overlap_linearspec/eval_margin_risk_multi_overlap.sh --mode overlap_lora --benchmarks gsm8k:1 --tokens 8192 --block-size 16 --margin-risk-threshold 0.5 --gpu-device auto --gpu-candidates 1,3,5 --gpu-min-free-gb 28 --gpu-wait-seconds 1800
```
...
```bash
bash method/margin_risk_multi_overlap_linearspec/eval_margin_risk_multi_overlap.sh --mode overlap_lora --benchmarks gsm8k:1 --max-samples 2 --tokens 512 --block-size 16 --margin-risk-threshold 0.5 --gpu-device 1 --port 19081 --output-path /data/home/wly/dLLM/NLD_results/margin_risk_multi_overlap_results --baseline-block16-dir /data/home/wly/dLLM/NLD_results/observations/pytorch_nemo_eval_results/eval_20260804_120138 --baseline-block32-dir /data/home/wly/dLLM/NLD_results/observations/pytorch_nemo_eval_results/eval_20260804_114935 --keep-runtime
```
...
```bash
python method/margin_risk_multi_overlap_linearspec/report.py --result-dir /data/home/wly/dLLM/NLD_results/margin_risk_multi_overlap_results/margin_risk_multi_overlap_YYYYMMDD_HHMMSS --baseline-block16-dir /data/home/wly/dLLM/NLD_results/observations/pytorch_nemo_eval_results/eval_20260804_120138 --baseline-block32-dir /data/home/wly/dLLM/NLD_results/observations/pytorch_nemo_eval_results/eval_20260804_114935
```
...
4. 漏报、预测后出错、P1/P2/P3 修正正确/错误、new 命中/未用的次数和比例；
5. 每数据集每 forward 的实际 dense query-token 数和分布；
6. 全部 13 个互斥状态的本轮/下一轮接收长度与差值；
7. 分区完整性和统计口径说明。
...
```bash
bash -n method/margin_risk_multi_overlap_linearspec/eval_margin_risk_multi_overlap.sh method/margin_risk_multi_overlap_linearspec/run_pipeline.sh
```

```bash
python -m py_compile method/margin_risk_multi_overlap_linearspec/*.py method/margin_risk_multi_overlap_linearspec/tests/*.py
```

离线单元测试：

```bash
/data/home/wly/.conda/envs/nld_sglang/bin/python -m unittest discover -s method/margin_risk_multi_overlap_linearspec/tests -v
```

真实模型四行融合 smoke：它分别验证“三候选+verifier”和“两候选+new+verifier”，比较 fused verifier 与普通 causal verifier 的 greedy token，检查 P1/P2/P3 prospective seed、continuation 分支、padding 计数和 canonical cache 不变。

```bash
CUDA_VISIBLE_DEVICES=3 /data/home/wly/.conda/envs/nld_sglang/bin/python method/margin_risk_multi_overlap_linearspec/tests/smoke_fused_multi.py --model /data1/linyewei/models/Nemotron-Labs-Diffusion-8B --lora-path /data1/linyewei/models/Nemotron-Labs-Diffusion-8B/linear_spec_lora --block-size 16 --dtype bfloat16
```
