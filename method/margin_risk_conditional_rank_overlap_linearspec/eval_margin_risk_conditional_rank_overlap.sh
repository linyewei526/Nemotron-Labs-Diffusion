#!/bin/bash
# Isolated fixed-margin-risk conditional-rank overlap + NeMo-Skills evaluation.
...
DEFAULT_OUTPUT_PATH="/data/home/wly/dLLM/NLD_results/margin_risk_conditional_rank_overlap_results"
...
DEFAULT_BENCHMARKS="gsm8k:1,human-eval:1,mbpp:1,math-500:1,aime24:1,aime25:1,gpqa:1,ifeval:1,livecodebench-cpp:1,mmlu:1"
...
Modes:
  --mode overlap_base        Margin-risk-conditional-rank-overlap LinearSpec without LoRA
  --mode overlap_lora        Margin-risk-conditional-rank-overlap LinearSpec with segmented draft LoRA
...
Margin-risk-conditional-rank-overlap decoding:
...
MODEL="$DEFAULT_MODEL"
SERVED_MODEL_NAME="nemotron-labs-diffusion-8b-margin-risk-conditional-rank-overlap"
TOKENS="8192"
TEMPERATURE="0"
TOP_P="0.95"
NUM_CHUNKS=""
...
case "$MODE" in
    overlap_base|margin_risk_conditional_rank_overlap_base) MODE="overlap_base" ;;
    overlap_lora|margin_risk_conditional_rank_overlap_lora) MODE="overlap_lora" ;;
...
if draft_threshold != 0.0:
    raise SystemExit("the conditional-rank overlap experiment requires --threshold 0")
...
if temperature != 0:
    raise SystemExit("the conditional-rank overlap experiment requires --temperature 0")
...
RUN_NAME="margin_risk_conditional_rank_overlap_$(date +%Y%m%d_%H%M%S)"
...
echo "================================================================"
echo " Margin-risk-conditional-rank-overlap LinearSpec + NeMo-Skills eval"
echo "================================================================"
echo " Mode:                 $MODE"
echo " Model:                $MODEL"
...
payload = {
    ...
    "status": "initialized",
    "experiment": "fixed margin-risk conditional-rank overlap: <=2 risks use per-position rank2 plus continuation; >=3 risks use P1-rank2, P1-rank3 and P2-rank2",
    "entrypoint": "method/margin_risk_conditional_rank_overlap_linearspec/eval_margin_risk_conditional_rank_overlap.sh",
    "original_args": sys.argv[2:],
    "command": "bash method/margin_risk_conditional_rank_overlap_linearspec/eval_margin_risk_conditional_rank_overlap.sh " + " ".join(shlex.quote(x) for x in sys.argv[2:]),
    "backend": "native_pytorch_margin_risk_conditional_rank_overlap",
    "benchmark": {
    ...
    "pytorch": {
        ...
        "candidate_allocation_rule": "if total crossings <=2: rank2 at every P1/P2 plus continuation; if total crossings >=3: P1 rank2, P1 rank3, P2 rank2, no P3 branch and no continuation",
        ...
        "alternative_token_rank_semantics": "rank2/rank3 among valid alternatives after excluding draft original token, MASK and EOS; the draft original token is treated as rank1",
...
export SEQ_EVAL_EXPNAME="margin_risk_conditional_rank_overlap"
...
if [[ "$PIPELINE_STATUS" != "0" ]]; then
    ...
    echo "ERROR: margin-risk-conditional-rank-overlap pipeline failed; internal work kept: $INTERNAL_JOB_DIR" >&2
...
"$PYTORCH_PYTHON" "$REPORT_SCRIPT" --result-dir "$FINAL_JOB_DIR" --baseline-block16-dir "$BASELINE_BLOCK16_DIR" --baseline-block32-dir "$BASELINE_BLOCK32_DIR" || true
echo "Completed margin-risk-conditional-rank-overlap LinearSpec + NeMo-Skills evaluation."
echo "Final output: $FINAL_JOB_DIR"
if [[ "$ANY_ERROR" != "0" ]]; then
