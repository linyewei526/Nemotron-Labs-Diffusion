#!/usr/bin/env python3
"""OpenAI-compatible server for fixed-margin-risk conditional-rank overlap."""
...
from method.margin_risk_conditional_rank_overlap_linearspec.generation import overlap_linear_spec_generate
from method.margin_risk_conditional_rank_overlap_linearspec.segmented_lora import install_segmented_lora
...
class MarginRiskConditionalRankOverlapEngine:
    ...
    def generate(
        ...
        if temperature != 0.0:
            raise ValueError("margin-risk-conditional-rank-overlap experiment currently requires temperature=0")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--served-model-name", default="nemotron-labs-diffusion-8b-margin-risk-conditional-rank-overlap")
...
def create_app(engine: MarginRiskConditionalRankOverlapEngine) -> FastAPI:
    app = FastAPI(title="NLD Margin-Risk-Conditional-Rank-Overlap LinearSpec Server", version="1.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
    ...
    async def health() -> dict[str, Any]:
        return {
            ...
            "candidate_policy": "risks<=2: P1/P2-rank2+new; risks>=3: P1-rank2/P1-rank3/P2-rank2",
        }
            ...
            "data": [
                {
                    ...
                    "object": "model",
                    "created": int(time.time()),
                    "owned_by": "local-margin-risk-conditional-rank-overlap",
                }
            ],
        }
    ...
    async def chat_completions(request: ChatCompletionRequest) -> dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Margin-risk-conditional-rank-overlap generation failed: {type(exc).__name__}: {exc}",
            ) from exc
        return {
            "id": request_id,
...
def main() -> int:
    args = parse_args()
    engine = MarginRiskConditionalRankOverlapEngine(args)
