#!/usr/bin/env python3
"""Merge fixed-margin-risk conditional-rank overlap stats into NeMo metrics."""
...
def summarize(rows: list[dict[str, Any]], wall_time_s: Optional[float]) -> dict[str, Any]:
    ...
    for field, keys in (
        ...
        (
            "policy_case_rounds",
            ("at_most_two_plus_new", "three_plus_ranked"),
        ),
    ...
    overlap["candidate_fixed_count"] = sum(
        ...
        for state in ("p1_rank2_fixed", "p1_rank3_fixed", "p2_rank2_fixed")
    )
    overlap["candidate_wrong_count"] = sum(
        int(outcome_states[state]["count"] or 0)
        for state in ("p1_all_candidates_wrong", "p2_rank2_wrong")
    )
    localized_states = (
        "p1_rank2_fixed", "p1_rank3_fixed", "p1_all_candidates_wrong",
        "p1_no_executable_candidate", "p2_rank2_fixed", "p2_rank2_wrong",
        "p2_no_executable_candidate", "p3_detected_no_candidate",
    )
...
def main() -> int:
    ...
    payload = {
        ...
        "metric_notes": {
            ...
            "outcome_states": "fifteen mutually-exclusive outcomes among actual conditional-rank overlap attempts, including locator misses, P1 rank2/rank3, P2 rank2, P3 locator-only, and continuation states",
            "candidate_correction": "branch-specific correctness when the verifier first rejects at that branch position; P1-rank2 and P1-rank3 are both checked in the three-plus-risk case",
    ...
    metrics["pytorch_margin_risk_conditional_rank_overlap"] = payload
    ...
    summary_path = (
        ...
        else metrics_path.parent / "pytorch_margin_risk_conditional_rank_overlap_metrics_summary.json"
    )
