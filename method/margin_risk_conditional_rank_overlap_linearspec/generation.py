#!/usr/bin/env python3
"""Fixed-margin-risk conditional-rank overlap LinearSpec generation.

Each fused call contains one padded causal verifier row and up to three
prospective rows.  With at most two strict risk crossings, each position gets
its valid rank-2 token and the remaining row continues the full block.  With
three or more crossings, the rows are P1-rank2, P1-rank3 and P2-rank2; P3 is a
locator-only signal and continuation is disabled.  Only the causal verifier
commits output and canonical KV state.
"""
...
CANDIDATE_BRANCH_KEYS = ("p1_rank2", "p1_rank3", "p2_rank2")
OUTCOME_STATES = (
...
class OverlapGenerationStats:
    ...
    policy_case_rounds: dict[str, int] = field(
        default_factory=lambda: {"at_most_two_plus_new": 0, "three_plus_ranked": 0}
    )
    ...
    def record_crossing_count(self, count: int) -> None:
        ...
        case = "at_most_two_plus_new" if count <= 2 else "three_plus_ranked"
    ...
    def record_outcome(self, state: str, accept_length: int) -> None:
        if state not in self.outcome_states:
            raise ValueError(f"unknown multi-overlap outcome state: {state}")
...
def _analyze_draft(
    *,
    ...
    # Candidate allocation is conditional on the *raw* crossing count, not on
    # later context/budget filtering.  P3 is deliberately locator-only in the
    # dense-risk case so that the three branch slots can test P1 rank2/rank3
    # and P2 rank2.
    requested: list[tuple[int, int, int, float]] = []
    if total_crossings <= 2:
        requested.extend(
            (rank, position, 2, risk)
            for rank, (position, risk) in enumerate(first_crossings, start=1)
        )
    else:
        p1_position, p1_risk = first_crossings[0]
        p2_position, p2_risk = first_crossings[1]
        requested.extend(
            [
                (1, p1_position, 2, p1_risk),
                (1, p1_position, 3, p1_risk),
                (2, p2_position, 2, p2_risk),
            ]
        )

    candidates: list[CandidateSpec] = []
    for rank, position, confidence_rank, risk in requested:
        alternative = _select_ranked_alternative(
            clean[position],
...
def _fused_verify_and_drafts(
    ...
    *,
    ...
    if rows > 4:
        raise RuntimeError("multi-overlap fused batch exceeds four rows")
...
def _classify_conditional_outcome(
    *,
    ...
    if not position_specs:
        ...
        return "p3_detected_no_candidate", None
    ...
    if matched_risk_rank == 1:
        return "p1_all_candidates_wrong", None
    if matched_risk_rank == 2:
...
def overlap_linear_spec_generate(
    ...
    *,
    ...
    if temperature != 0.0:
        raise ValueError("The isolated multi-overlap experiment supports temperature=0 only")
    if draft_threshold != 0.0:
        raise ValueError("The isolated multi-overlap experiment requires draft_threshold=0")
