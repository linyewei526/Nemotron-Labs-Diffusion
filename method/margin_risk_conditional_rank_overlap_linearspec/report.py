#!/usr/bin/env python3
"""Incrementally render the fixed-margin-risk conditional-rank report."""
...
def decode(payload: dict[str, Any], *, new_method: bool) -> dict[str, Any]:
    key = "pytorch_margin_risk_conditional_rank_overlap" if new_method else "pytorch_native"
...
def render(result_dir: Path, baseline16: Path, baseline32: Path) -> str:
    ...
    lines = [
        "# 固定 margin-risk=0.5 条件式 rank 候选 overlap 实验报告",
        "",
        ...
        "## 1. 配置与 baseline",
        "",
        "变量说明：`BS` 是 block size；`MR阈` 是严格触发阈值；新方法最多使用 1 条 verifier row 加 3 条 speculative row。风险位置不超过 2 个时使用各位置二选候选并加 new；达到 3 个时改用 P1二选、P1三选、P2二选且不加 new。B16/B32 是既有 PyTorch+NeMo-Skills、LinearSpec LoRA、greedy、block size 16/32 baseline。示例：MR阈为 0.5 时，等于 0.5 不触发。",
        "",
        table(
    ...
    lines.extend([
        ...
        "## 3. Crossing、分支与复用漏斗",
        "",
        "变量说明：`C0/C1/C2/C3+` 是当前 draft 中严格 crossing 数量对应的轮数；`少险/多险` 分别是 crossing≤2 的“各位置二选+new”模式和 crossing≥3 的“P1二选+P1三选+P2二选”模式。`R2/R3/R4` 是 fused forward 的总 row 数；`P1二B/P1三B/P2二B` 是实际执行的三类纠错分支数；`NewB` 是 continuation 分支数；`验中/可复用/已复用` 依次是 verifier 确认命中、通过边界检查后可留用以及下一轮实际消费。示例：C2 通常是 verifier+P1二选+P2二选+new，共 R4；C3+ 通常是 verifier+三类 rank 候选，也为 R4。",
        "",
    ])
    ...
    for dataset in completed:
        ...
        funnel_rows.append([
            ...
            fmt(cases.get("at_most_two_plus_new")),
            fmt(cases.get("three_plus_ranked")),
            fmt(ov.get("prefetch_attempts")), fmt(rows_hist.get("2")),
            fmt(rows_hist.get("3")), fmt(rows_hist.get("4")),
            fmt(branches.get("p1_rank2")), fmt(branches.get("p1_rank3")),
            fmt(branches.get("p2_rank2")),
    ...
    if completed:
        ...
        funnel_rows.append([
            ...
            *[fmt(total_nested("policy_case_rounds", key)) for key in ("at_most_two_plus_new", "three_plus_ranked")],
            ...
            *[fmt(total_nested("candidate_branch_executed", key)) for key in ("p1_rank2", "p1_rank3", "p2_rank2")],
        ...
        funnel_rows.append([
            ...
            *[fmt(mean(integer((overlap(new_metrics[d]).get("policy_case_rounds") or {}).get(key)) for d in completed)) for key in ("at_most_two_plus_new", "three_plus_ranked")],
            ...
            *[fmt(mean(integer((overlap(new_metrics[d]).get("candidate_branch_executed") or {}).get(key)) for d in completed)) for key in ("p1_rank2", "p1_rank3", "p2_rank2")],
            *[fmt(mean(integer(overlap(new_metrics[d]).get(field)) for d in completed)) for field in funnel_fields[2:]],
        ])
        lines.append(table(
            ["数据集", "轮数", "C0", "C1", "C2", "C3+", "少险", "多险", "融合轮", "R2", "R3", "R4", "P1二B", "P1三B", "P2二B", "CandB", "NewB", "验中", "可复用", "已复用"],
            funnel_rows,
        ))
    else:
    ...
    lines.extend([
        ...
        "## 4. 关键状态计数与占比",
        "",
        "变量说明：每格为“次数/占实际融合轮比例”。`漏报` 表示首错不在前三个风险位置；`预测后` 是其中首错位于最后风险位置之后的子集。`P1二中/P1三中` 表示首错恰在 P1 且对应 confidence rank 的 token 修正成功；`P1全错` 表示本轮为 P1 准备的一条或两条候选均不正确。`P2二中/P2二错` 对应 P2 的 rank-2 分支；`P3仅定` 表示成功定位到 P3，但按策略不为 P3 构造分支；`边界跳` 是 P1/P2 已定位但候选因 token/context/thinking 边界不可执行。`new未用` 合并整块通过但 new 未命中或多风险模式没有 new。示例：P1三中=5/10% 表示 5 轮由 P1 的第三置信 token 修正成功，占融合轮 10%。",
        "",
    ])
    key_rows: list[list[Any]] = []
    macro_key: dict[str, list[float]] = {
        key: [] for key in (
            "miss", "after", "p1r2", "p1r3", "p1wrong", "p2r2",
            "p2wrong", "p3only", "boundary", "newhit", "newunused",
        )
    }
    ...
    for dataset in completed:
        ...
        counts = {
            ...
            "p1r2": integer(state_item(payload, "p1_rank2_fixed").get("count")),
            "p1r3": integer(state_item(payload, "p1_rank3_fixed").get("count")),
            "p1wrong": integer(state_item(payload, "p1_all_candidates_wrong").get("count")),
            "p2r2": integer(state_item(payload, "p2_rank2_fixed").get("count")),
            "p2wrong": integer(state_item(payload, "p2_rank2_wrong").get("count")),
            "p3only": integer(state_item(payload, "p3_detected_no_candidate").get("count")),
    ...
    if completed:
        ...
        lines.append(table(
            ["数据集", "漏报", "预测后", "P1二中", "P1三中", "P1全错", "P2二中", "P2二错", "P3仅定", "边界跳", "new命中", "new未用"],
            key_rows,
        ))
    else:
    ...
    lines.extend([
        ...
        "",
        "变量说明：`检查` 表示 verifier 首错恰在该分支对应风险位置、因而能够判断替代 token 对错的次数；`修对/修错` 是该替代 token 等于/不等于 verifier token 的次数；`修对率=修对/检查`。P1二选与 P1三选会在多风险模式的同一轮同时接受检查，因此此表不是互斥状态表。例如 P1三选修对时，同轮 P1二选必然计为修错。某数据集检查为 0 时条件正确率未定义，不用 0 代填。",
        "",
    ])
    ...
    lines.extend([
        ...
        "",
        "变量说明：下表保留全部 15 个互斥状态，区分 P1 的 rank-2/rank-3 命中、P2 rank-2、P3 仅定位、候选不可执行、漏报和 continuation。`Cnt` 是状态次数；`占比` 是 Cnt/融合轮；`本轮均` 是连续匹配 draft token 数加 verifier bonus 后的平均接收长度；`NextN/NextCov` 是确实存在同一 request 下一 verify 的数量/覆盖率；`配对本轮` 只在这些配对样本上计算；`下轮均` 是下一轮接收均值；`差值` 是逐对“下轮−当前轮”的均值。终止轮不补成 0。例如 P3命中但无分支仍能观察其下一轮接收能力，但不会计作本轮纠错命中。",
        "",
    ])
