#!/usr/bin/env python3
"""OpenAI-compatible server for the fixed-margin-risk no-P3 overlap method."""
...
from method.margin_risk_no_p3_overlap_linearspec.generation import overlap_linear_spec_generate
from method.margin_risk_no_p3_overlap_linearspec.segmented_lora import install_segmented_lora
...
class MarginRiskNoP3OverlapEngine:
    ...
    def generate(
        ...
        if temperature != 0.0:
            raise ValueError("margin-risk-no-p3-overlap experiment currently requires temperature=0")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--served-model-name", default="nemotron-labs-diffusion-8b-margin-risk-no-p3-overlap")
...
def create_app(engine: MarginRiskNoP3OverlapEngine) -> FastAPI:
    app = FastAPI(title="NLD Margin-Risk-No-P3-Overlap LinearSpec Server", version="1.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
    ...
    async def health() -> dict[str, Any]:
        return {
            ...
            "max_speculative_rows": 3,
            "candidate_policy": "C0:new; C1:P1+new; C2:P1+P2+new; C3+:P1+P2",
        }
            ...
            "data": [
                {
                    ...
                    "object": "model",
                    "created": int(time.time()),
                    "owned_by": "local-margin-risk-no-p3-overlap",
                }
            ],
        }
    ...
    async def chat_completions(request: ChatCompletionRequest) -> dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Margin-risk-no-P3-overlap generation failed: {type(exc).__name__}: {exc}",
            ) from exc
        return {
            "id": request_id,
...
def main() -> int:
    args = parse_args()
    engine = MarginRiskNoP3OverlapEngine(args)
