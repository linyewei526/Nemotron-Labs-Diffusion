#!/usr/bin/env python3
"""Fixed-margin-risk no-P3 overlap LinearSpec generation.

The raw number of strict margin-risk crossings chooses a fixed row policy:
C0 -> New, C1 -> P1+New, C2 -> P1+P2+New, C3+ -> P1+P2.  P3 is never executed;
it is retained only as a locator-only audit signal.  Only the causal verifier
row commits output and canonical KV state.
"""
...
CANDIDATE_BRANCH_KEYS = ("p1_rank2", "p2_rank2")
...
class OverlapGenerationStats:
    ...
    def record_outcome(self, state: str, accept_length: int) -> None:
        if state not in self.outcome_states:
            raise ValueError(f"unknown no-P3 overlap outcome state: {state}")
...
def _analyze_draft(
    *,
    ...
    # P1/P2 always get exactly one rank-2 correction row when they exist.  P3
    # is deliberately locator-only so C3+ needs three rows total, not four.
    requested = [
        (rank, position, 2, risk)
        for rank, (position, risk) in enumerate(first_crossings[:2], start=1)
    ]
...
def _fused_verify_and_drafts(
    ...
    *,
    ...
    if rows > 4:
        raise RuntimeError("no-P3 overlap fused batch exceeds four rows")
...
def _classify_conditional_outcome(
    *,
    ...
    correctly located without having an executable branch (P3 by design, or a
    P1/P2 candidate removed by a generation/context/thinking boundary).
    """
    ...
    if not position_specs:
        ...
        if matched_risk_rank == 2:
            return "p2_no_executable_candidate", None
        alternative = (
            risk_rank2_tokens[2] if len(risk_rank2_tokens) >= 3 else None
        )
    ...
    if matched_risk_rank == 2:
        return "p2_rank2_wrong", None
    return "p3_omitted_rank2_wrong", None
...
def overlap_linear_spec_generate(
    ...
    *,
    ...
    if prompt_ids.shape[0] != 1:
        raise ValueError("No-P3 overlap LinearSpec requires request batch_size == 1")
    ...
    if temperature != 0.0:
        raise ValueError("The isolated no-P3 overlap experiment supports temperature=0 only")
    if draft_threshold != 0.0:
        raise ValueError("The isolated no-P3 overlap experiment requires draft_threshold=0")
