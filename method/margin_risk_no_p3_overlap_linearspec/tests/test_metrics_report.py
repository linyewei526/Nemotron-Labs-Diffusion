...
from method.margin_risk_no_p3_overlap_linearspec.merge_metrics import main, summarize
from method.margin_risk_no_p3_overlap_linearspec.report import (
    DEFAULT_BASELINE_16,
    DEFAULT_BASELINE_32,
    decode,
    report_datasets,
...
from method.margin_risk_no_p3_overlap_linearspec.server import (
    ChatCompletionRequest,
    MarginRiskNoP3OverlapEngine,
)
...
class MetricsAndReportTests(unittest.TestCase):
    ...
    def test_merge_keeps_no_p3_states_and_dense_forward_distribution(self) -> None:
        ...
        self.assertEqual(overlap["policy_case_rounds"]["c3plus_p1_p2"], 1)
        self.assertEqual(overlap["candidate_branch_executed"]["p1_rank2"], 2)
        self.assertEqual(overlap["candidate_correction"]["p2_rank2"]["fixed_rate"], 1)
        ...
        self.assertEqual(overlap["decode_dense_query_token_slots_total"], 256)
        self.assertEqual(overlap["decode_dense_query_token_slots_per_forward"], 128)
        self.assertEqual(overlap["decode_dense_query_token_slots_per_output_token"], 32)
    ...
    def test_merge_can_create_efficiency_only_metrics_when_scorer_failed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            wrapper = payload["pytorch_margin_risk_no_p3_overlap"]
    ...
    def test_engine_turns_oom_into_failed_stat_and_empty_placeholder(self) -> None:
        engine = MarginRiskNoP3OverlapEngine.__new__(MarginRiskNoP3OverlapEngine)
        ...
        with (
            patch(
                "method.margin_risk_no_p3_overlap_linearspec.server.overlap_linear_spec_generate",
                side_effect=oom,
            ),
    ...
    def test_initial_report_exists_before_any_dataset_finishes(self) -> None:
        ...
        self.assertIn("P1/P2 二选", content)

    def test_report_renders_no_p3_states_and_dense_slots(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            payload = {
                ...
                "pytorch_margin_risk_no_p3_overlap": {
                    "decode": {
                        ...
                        "overlap": {
                            ...
                            "decode_dense_query_token_slots_total": 256,
                            "decode_dense_query_token_slots_per_forward": 128,
                            "decode_dense_query_token_slots_per_output_token": 16,
