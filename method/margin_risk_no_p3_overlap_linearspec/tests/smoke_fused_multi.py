#!/usr/bin/env python3
"""Real-model smoke for no-P3 three/four-row fused paths."""
...
from method.margin_risk_no_p3_overlap_linearspec.generation import (
    CandidateSpec,
    DraftState,
...
from method.margin_risk_no_p3_overlap_linearspec.hybrid import repeat_dynamic_cache
...
def main() -> int:
    ...
    specs = (
        forced_spec(draft, 1, positions[0], vocab_size, excluded, 2),
        forced_spec(draft, 2, positions[1], vocab_size, excluded, 2),
    )
    forced = DraftState(
        tokens=draft.tokens,
    ...
    if not torch.equal(candidate_logits.argmax(dim=-1), expected):
        raise RuntimeError("three-row candidate verifier differs from normal causal verify")
    if candidate_new is not None or set(candidate_drafts) != {"p1_rank2", "p2_rank2"}:
        raise RuntimeError("candidate-only fused path returned incorrect branches")
    if candidate_stats.processed_rows != 3:
        raise RuntimeError("C3+ no-P3 fused path must contain verifier+P1+P2")
    for spec in specs:
    ...
    if set(continuation_drafts) != {"p1_rank2", "p2_rank2"} or continuation_new is None:
        raise RuntimeError("candidate+continuation fused path returned incorrect branches")
    if continuation_stats.processed_rows != 4:
        raise RuntimeError("C2 fused path must contain verifier+P1+P2+new")
