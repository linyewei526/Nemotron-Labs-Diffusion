#!/usr/bin/env python3
"""Merge fixed-margin-risk no-P3 overlap stats into NeMo metrics."""
...
def summarize(rows: list[dict[str, Any]], wall_time_s: Optional[float]) -> dict[str, Any]:
    ...
    for field, keys in (
        ...
        (
            "policy_case_rounds",
            ("c0_new", "c1_p1_new", "c2_p1_p2_new", "c3plus_p1_p2"),
        ),
    ...
    overlap["candidate_fixed_count"] = sum(
        ...
        for state in ("p1_rank2_fixed", "p2_rank2_fixed")
    )
    overlap["candidate_wrong_count"] = sum(
        int(outcome_states[state]["count"] or 0)
        for state in ("p1_rank2_wrong", "p2_rank2_wrong")
    )
    localized_states = (
        "p1_rank2_fixed", "p1_rank2_wrong",
        "p1_no_executable_candidate", "p2_rank2_fixed", "p2_rank2_wrong",
        "p2_no_executable_candidate", "p3_omitted_rank2_would_fix",
        "p3_omitted_rank2_wrong",
    )
...
def main() -> int:
    ...
    payload = {
        ...
        "metric_notes": {
            ...
            "outcome_states": "fifteen mutually-exclusive no-P3 outcomes: locator misses, executed P1/P2 rank2 branches, omitted-P3 counterfactual audit, and continuation states",
            "candidate_correction": "branch-specific correctness when the verifier first rejects at an executed P1/P2 position",
    ...
    metrics["pytorch_margin_risk_no_p3_overlap"] = payload
    ...
    summary_path = (
        ...
        else metrics_path.parent / "pytorch_margin_risk_no_p3_overlap_metrics_summary.json"
    )
