#!/usr/bin/env python3
"""Incrementally render the fixed-margin-risk P1-conditional-new overlap report."""
...
STATE_ZH = {
    ...
    "p2_omitted_rank2_would_fix": "P2省略但二选本可修正",
    "p2_omitted_rank2_wrong": "P2省略且二选也会错",
...
def decode(payload: dict[str, Any], *, new_method: bool) -> dict[str, Any]:
    key = "pytorch_margin_risk_p1_conditional_new_overlap" if new_method else "pytorch_native"
...
def render(result_dir: Path, baseline16: Path, baseline32: Path) -> str:
    ...
    lines = [
        "# 固定 margin-risk=0.5 P1 + conditional-new overlap 实验报告",
        "",
        ...
        "## 1. 配置与 baseline",
        "",
        "变量说明：`BS` 是 block size；`MR阈` 是严格触发阈值。分支规则为 C0→new、C1→P1+new、C2→P1+new、C3+→P1；P2/P3 永不执行，只做反事实审计。加 verifier 后最多 3 row。B16/B32 是既有 PyTorch+NeMo-Skills、LinearSpec LoRA、greedy、block size 16/32 baseline。示例：MR阈为 0.5 时，等于 0.5 不触发。",
        "",
        table(
    ...
    lines.extend([
        ...
        "## 3. Crossing、分支与复用漏斗",
        "",
        "变量说明：`C0/C1/C2/C3+` 是当前 draft 中严格 crossing 的数量；同名策略列是实际采用该分支配置的轮数。`R2/R3` 是 fused forward 的总 row 数；`P1B/NewB` 是实际执行的 P1、continuation 分支数；`验中/可复用/已复用` 依次是 verifier 确认种子命中、通过结束边界后可留用、下一轮实际消费。示例：C2 是 verifier+P1+new，共 R3；C3+ 是 verifier+P1，共 R2。",
        "",
    ])
    ...
    for dataset in completed:
        ...
        funnel_rows.append([
            ...
            fmt(cases.get("c2_p1_new")), fmt(cases.get("c3plus_p1")),
            fmt(ov.get("prefetch_attempts")), fmt(rows_hist.get("2")),
            fmt(rows_hist.get("3")), fmt(branches.get("p1_rank2")),
    ...
    if completed:
        ...
        funnel_rows.append([
            ...
            *[fmt(total_nested("policy_case_rounds", key)) for key in ("c0_new", "c1_p1_new", "c2_p1_new", "c3plus_p1")],
            fmt(sum(integer(overlap(new_metrics[d]).get("prefetch_attempts")) for d in completed)),
            *[fmt(total_nested("fused_row_count", key)) for key in ("2", "3")],
            fmt(total_nested("candidate_branch_executed", "p1_rank2")),
        ...
        funnel_rows.append([
            ...
            *[fmt(mean(integer((overlap(new_metrics[d]).get("policy_case_rounds") or {}).get(key)) for d in completed)) for key in ("c0_new", "c1_p1_new", "c2_p1_new", "c3plus_p1")],
            fmt(mean(integer(overlap(new_metrics[d]).get("prefetch_attempts")) for d in completed)),
            *[fmt(mean(integer((overlap(new_metrics[d]).get("fused_row_count") or {}).get(key)) for d in completed)) for key in ("2", "3")],
            fmt(mean(integer((overlap(new_metrics[d]).get("candidate_branch_executed") or {}).get("p1_rank2")) for d in completed)),
            *[fmt(mean(integer(overlap(new_metrics[d]).get(field)) for d in completed)) for field in funnel_fields[2:]],
        ])
        lines.append(table(
            ["数据集", "轮数", "C0", "C1", "C2", "C3+", "策C0", "策C1", "策C2", "策C3+", "融合轮", "R2", "R3", "P1B", "CandB", "NewB", "验中", "可复用", "已复用"],
            funnel_rows,
        ))
    else:
    ...
    lines.extend([
        ...
        "## 4. 关键状态计数与占比",
        "",
        "变量说明：每格为“次数/占实际融合轮比例”。`漏报` 表示首错不在前三个风险位置；`预测后` 是首错位于最后风险位置之后的子集。`P1中/错` 是实际执行 P1 分支的修正结果。`P2本可修/也会错` 与 `P3本可修/也会错` 是未执行分支的反事实审计，不算实际复用；例如 P2本可修=5/1% 表示 1% 融合轮省掉了一个本可命中的 P2 row。`边界跳` 只表示 P1 已定位但受 token/context/thinking 边界限制无法执行。`new未用` 合并整块通过但 new 未命中或 C3+ 没有 new。",
        "",
    ])
    key_rows: list[list[Any]] = []
    macro_key: dict[str, list[float]] = {
        key: [] for key in (
            "miss", "after", "p1fixed", "p1wrong", "p2would",
    ...
    for dataset in completed:
        ...
        counts = {
            ...
            "p2would": integer(state_item(payload, "p2_omitted_rank2_would_fix").get("count")),
            "p2wrong": integer(state_item(payload, "p2_omitted_rank2_wrong").get("count")),
            ...
            "boundary": integer(state_item(payload, "p1_no_executable_candidate").get("count")),
    ...
    if completed:
        ...
        lines.append(table(
            ["数据集", "漏报", "预测后", "P1中", "P1错", "P2本可修", "P2也会错", "P3本可修", "P3也会错", "边界跳", "new命中", "new未用"],
            key_rows,
        ))
    else:
    ...
    lines.extend([
        ...
        "",
        "变量说明：`检查` 表示 verifier 首错恰在唯一实际执行的 P1 分支位置、因而能够判断二选 token 对错的次数；`修对/修错` 是该 token 等于/不等于 verifier token 的次数；`修对率=修对/检查`。某数据集检查为 0 时条件正确率未定义，不用 0 代填。P2/P3 反事实单列在上一表，不混入实际分支正确率。",
        "",
    ])
    ...
    lines.extend([
        ...
        "### 5.1 Forward slot 分布",
        "",
        "变量说明：`FwdTok` 是 dense forward 实际送入模型的 query token slot，即 row数×统一 padding 后 Q；`有效均` 是各 row 有效长度之和的平均；`Pad均` 是 FwdTok−有效 token；`Pad率` 是总 padding slot/总 FwdTok；`Rows均/Q均` 是平均 row 数和平均公共 Q。`全部` 包含 prefill，`解码` 排除 prefill，`融合` 只统计多行 fused forward。示例：3 row、Q=32 时 FwdTok=96，即使部分位置被 attention mask 屏蔽，这些 padding token 仍经过 dense QKV/MLP 计算。",
        "",
    ])
    ...
    lines.extend([
        ...
        "",
        "变量说明：下表保留全部 14 个互斥状态，区分实际 P1 二选结果、P2/P3 省略分支的反事实结果、P1 候选不可执行、漏报和 continuation。`Cnt` 是状态次数；`占比` 是 Cnt/融合轮；`本轮均` 是连续匹配 draft token 数加 verifier bonus 后的平均接收长度；`NextN/NextCov` 是确实存在同一 request 下一 verify 的数量/覆盖率；`配对本轮` 只在这些配对样本上计算；`下轮均` 是下一轮接收均值；`差值` 是逐对“下轮−当前轮”的均值。终止轮不补成 0。",
        "",
    ])
    ...
    lines.extend([
        ...
        "- 14 个细状态必须互斥且次数之和等于实际 fused overlap 轮数；报告读取 outcome_partition_valid 校验。",
        ...
        "- TPS 受 GPU、显存预留和并发环境影响；TPF 也会把最多三行的 fused forward 计为一次，必须和 FwdTok/Pad率共同解读。",
        "",
    ])
    return "\n".join(lines)
