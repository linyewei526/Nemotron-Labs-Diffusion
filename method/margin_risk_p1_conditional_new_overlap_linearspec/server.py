#!/usr/bin/env python3
"""OpenAI-compatible server for the fixed-margin-risk P1-conditional-new overlap method."""
...
from method.margin_risk_p1_conditional_new_overlap_linearspec.generation import overlap_linear_spec_generate
from method.margin_risk_p1_conditional_new_overlap_linearspec.segmented_lora import install_segmented_lora
...
def _is_cuda_oom(exc: BaseException) -> bool:
    return isinstance(exc, torch.cuda.OutOfMemoryError) or (
        isinstance(exc, RuntimeError) and "cuda out of memory" in str(exc).lower()
    )


class MarginRiskP1ConditionalNewOverlapEngine:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.margin_risk_threshold = float(args.margin_risk_threshold)
        ...
        self.efficiency_only = bool(args.efficiency_only)
    ...
    def generate(
        ...
        if temperature != 0.0:
            raise ValueError("margin-risk-p1-conditional-new-overlap experiment currently requires temperature=0")
            ...
            try:
                ...
                with torch.inference_mode():
                    output_ids, decode_stats = overlap_linear_spec_generate(
                        ...
                        margin_risk_threshold=self.margin_risk_threshold,
                ...
                self._append_stat(
                    {
                        ...
                        "error": str(exc),
                        "oom_skipped_for_efficiency": bool(is_oom and self.efficiency_only),
                        "traceback_tail": traceback_tail,
                    }
                )
                if is_oom and self.efficiency_only:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--served-model-name", default="nemotron-labs-diffusion-8b-margin-risk-p1-conditional-new-overlap")
    ...
    parser.add_argument("--margin-risk-threshold", type=float, default=0.5)
    ...
    parser.add_argument(
        "--efficiency-only",
        action="store_true",
        help="turn request OOM into an empty placeholder and keep efficiency evaluation running",
    )
    ...
    if not 0 <= args.margin_risk_threshold <= 1:
        parser.error("--margin-risk-threshold must be in [0,1]")
    return args


def create_app(engine: MarginRiskP1ConditionalNewOverlapEngine) -> FastAPI:
    app = FastAPI(title="NLD Margin-Risk-P1-Conditional-New-Overlap LinearSpec Server", version="1.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
    ...
    async def health() -> dict[str, Any]:
        return {
            ...
            "margin_risk_threshold": engine.margin_risk_threshold,
            "efficiency_only": engine.efficiency_only,
            "max_total_rows": 3,
            "max_candidate_rows": 1,
            "max_speculative_rows": 2,
            "candidate_policy": "C0:new; C1:P1+new; C2:P1+new; C3+:P1",
        }
            ...
            "data": [
                {
                    ...
                    "object": "model",
                    "created": int(time.time()),
                    "owned_by": "local-margin-risk-p1-conditional-new-overlap",
                }
            ],
        }
    ...
    async def chat_completions(request: ChatCompletionRequest) -> dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Margin-risk-P1-conditional-new-overlap generation failed: {type(exc).__name__}: {exc}",
            ) from exc
        return {
            "id": request_id,
...
def main() -> int:
    args = parse_args()
    engine = MarginRiskP1ConditionalNewOverlapEngine(args)
