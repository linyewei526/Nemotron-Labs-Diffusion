#!/usr/bin/env python3
"""Fixed-margin-risk P1 + conditional-new overlap LinearSpec generation.

The raw number of strict margin-risk crossings chooses a fixed row policy:
C0 -> New, C1 -> P1+New, C2 -> P1+New, C3+ -> P1.  P2/P3 are never executed;
they are retained only as locator-only counterfactual audit signals.  Only the
causal verifier row commits output and canonical KV state.
"""
...
CANDIDATE_BRANCH_KEYS = ("p1_rank2",)
PREFETCH_SOURCE_KEYS = (*CANDIDATE_BRANCH_KEYS, "continuation")
OUTCOME_STATES = (
    ...
    "p2_omitted_rank2_would_fix",
    "p2_omitted_rank2_wrong",
...
class CandidateSpec:
    ...
    margin_risk: float
    confidence_rank: int = 2

    @property
    def branch_key(self) -> str:
        return f"p{self.rank}_rank{self.confidence_rank}"
...
class DraftState:
    ...
    source: str
    risk_positions: tuple[int, ...] = ()
    risk_rank2_tokens: tuple[Optional[int], ...] = ()


@dataclass
class CandidateCorrectionStats:
    checked: int = 0
    fixed: int = 0
    wrong: int = 0
    ...
    def to_dict(self) -> dict[str, int | float | None]:
        return {
            **asdict(self),
            "fixed_rate": self.fixed / self.checked if self.checked else None,
            "wrong_rate": self.wrong / self.checked if self.checked else None,
        }
...
class OverlapGenerationStats:
    ...
    continuation_not_added_many_crossings: int = 0
    ...
    fused_row_count: dict[str, int] = field(default_factory=lambda: {"2": 0, "3": 0})
    policy_case_rounds: dict[str, int] = field(
    ...
    candidate_branch_executed: dict[str, int] = field(
        default_factory=lambda: {name: 0 for name in CANDIDATE_BRANCH_KEYS}
    )
    candidate_correction: dict[str, CandidateCorrectionStats] = field(
        default_factory=lambda: {
            name: CandidateCorrectionStats() for name in CANDIDATE_BRANCH_KEYS
        }
    )
    prefetch_verified_hits_by_source: dict[str, int] = field(
        default_factory=lambda: {name: 0 for name in PREFETCH_SOURCE_KEYS}
    )
    prefetch_hits_by_source: dict[str, int] = field(
        default_factory=lambda: {name: 0 for name in PREFETCH_SOURCE_KEYS}
    )
    ...
    def record_crossing_count(self, count: int) -> None:
        ...
        self.policy_case_rounds[case] += 1
    ...
    def record_candidate_checks(
        ...
        *,
        ...
        for spec in candidate_specs:
            if spec.position != mismatch_position:
                continue
            correct = (
                int(ar_tokens[spec.position - 1].item()) == spec.alternative_token
            )
            self.candidate_correction[spec.branch_key].record(correct)

    def record_outcome(self, state: str, accept_length: int) -> None:
        if state not in self.outcome_states:
            raise ValueError(f"unknown P1-conditional-new overlap outcome state: {state}")
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        payload["candidate_correction"] = {
            name: self.candidate_correction[name].to_dict()
            for name in CANDIDATE_BRANCH_KEYS
        }
...
def _select_ranked_alternative(
    clean_logits: torch.Tensor,
    selected_token: int,
    excluded_token_ids: tuple[int, ...],
    confidence_rank: int,
) -> Optional[int]:
    """Select valid rank 2/3 relative to the original draft token as rank 1."""

    if confidence_rank < 2:
        raise ValueError("confidence_rank must be at least 2")
    ...
    alternative_index = confidence_rank - 2
    available = int(torch.isfinite(alternative_logits).sum().item())
    if available <= alternative_index:
        return None
    alternatives = torch.topk(
        alternative_logits, k=alternative_index + 1, dim=-1
    ).indices
    alternative = int(alternatives[alternative_index].item())
...
def _analyze_draft(
    *,
    ...
    """Find the first three strict margin-risk crossings after MASK exclusion."""
    ...
    first_crossings: list[tuple[int, float]] = []
    total_crossings = 0
    for position in range(1, tokens.shape[0]):
        ...
        if total_crossings <= 3:
            first_crossings.append((position, risk))

    # Only P1 gets a rank-2 correction row.  P2/P3 stay locator-only so every
    # fused call uses at most verifier + P1 + New (three rows total).
    requested = [
        (rank, position, 2, risk)
        for rank, (position, risk) in enumerate(first_crossings[:1], start=1)
    ]

    candidates: list[CandidateSpec] = []
    rank2_by_risk: list[Optional[int]] = []
    for position, _risk in first_crossings:
        rank2_by_risk.append(
            _select_ranked_alternative(
                clean[position],
    ...
    for rank, position, confidence_rank, risk in requested:
        alternative = rank2_by_risk[rank - 1]
    ...
    return DraftState(
        ...
        source=source,
        risk_positions=tuple(position for position, _risk in first_crossings),
        risk_rank2_tokens=tuple(rank2_by_risk),
    )
...
def _fused_verify_and_drafts(
    ...
    *,
    ...
    if len(candidate_specs) + int(include_continuation) > MAX_CANDIDATES:
        raise ValueError("speculative branch count exceeds two")
    ...
    if rows > 3:
        raise RuntimeError("P1-conditional-new overlap fused batch exceeds three rows")
    ...
    prospective_by_branch: dict[str, DraftState] = {}
    for row_index, spec in enumerate(candidate_specs, start=1):
        start = spec.position
        ...
        prospective_by_branch[spec.branch_key] = _analyze_draft(
            logits=prospective_logits,
            tokens=prospective_ids,
            mask_token_id=mask_token_id,
            excluded_alternative_token_ids=excluded_alternative_token_ids,
            margin_risk_threshold=margin_risk_threshold,
            source=f"candidate_{spec.branch_key}",
        )
    ...
    return verify_logits, output.past_key_values, prospective_by_branch, continuation
...
def _classify_conditional_outcome(
    *,
    mismatch_position: Optional[int],
    risk_positions: tuple[int, ...],
    risk_rank2_tokens: tuple[Optional[int], ...],
    ...
    """Return one mutually-exclusive outcome and the reusable branch key.

    ``continuation`` denotes the continuation branch.  A risk position can be
    correctly located without having an executable branch (P2/P3 by design,
    or a P1 candidate removed by a generation/context/thinking boundary).
    """
    ...
    if mismatch_position is None:
        ...
        if int(continuation.tokens[0].item()) == bonus:
            return "full_continuation_hit", "continuation"
        return "full_continuation_miss", None

    if not risk_positions:
        return "miss_no_risk_position", None
    if mismatch_position < risk_positions[0]:
        return "miss_before_first", None
    if mismatch_position > risk_positions[-1]:
        return "miss_after_last", None

    matched_risk_rank: Optional[int] = None
    for rank, position in enumerate(risk_positions, start=1):
        if mismatch_position == position:
            matched_risk_rank = rank
            break
    if matched_risk_rank is None:
        return "miss_between_positions", None

    position_specs = [
        spec for spec in candidate_specs if spec.position == mismatch_position
    ]
    if not position_specs:
        if matched_risk_rank == 1:
            return "p1_no_executable_candidate", None
        alternative_index = matched_risk_rank - 1
        alternative = (
            risk_rank2_tokens[alternative_index]
            if len(risk_rank2_tokens) > alternative_index
            else None
        )
        correct = alternative is not None and (
            int(ar_tokens[mismatch_position - 1].item()) == int(alternative)
        )
        if matched_risk_rank == 2:
            return (
                "p2_omitted_rank2_would_fix"
                if correct
                else "p2_omitted_rank2_wrong",
                None,
            )
        return (
            "p3_omitted_rank2_would_fix"
            if correct
            else "p3_omitted_rank2_wrong",
            None,
        )

    for spec in sorted(position_specs, key=lambda item: item.confidence_rank):
        correct = (
            int(ar_tokens[spec.position - 1].item()) == spec.alternative_token
        )
        if correct:
            return f"p{spec.rank}_rank{spec.confidence_rank}_fixed", spec.branch_key

    if matched_risk_rank == 1:
        return "p1_rank2_wrong", None
    raise RuntimeError("Only the P1 correction branch may be executable")
...
def overlap_linear_spec_generate(
    ...
    *,
    ...
    """Generate with at most three padded rows per fused overlap forward."""

    if prompt_ids.shape[0] != 1:
        raise ValueError("P1-Conditional-New overlap LinearSpec requires request batch_size == 1")
    ...
    if temperature != 0.0:
        raise ValueError("The isolated P1-conditional-new overlap experiment supports temperature=0 only")
    if draft_threshold != 0.0:
        raise ValueError("The isolated P1-conditional-new overlap experiment requires draft_threshold=0")
    ...
    while total_gen < max_new_tokens:
        ...
        if draft.total_crossing_count <= 2:
            ...
            stats.continuation_not_added_many_crossings += 1
        ...
        if can_overlap:
            ...
            for spec in candidate_specs:
                stats.candidate_branch_executed[spec.branch_key] += 1
                stats.candidate_position_sum += spec.position
                stats.candidate_position_count += 1
            verify_logits, fused_cache, prospective_by_branch, continuation = (
        ...
        if can_overlap:
            ...
            outcome_state, selected_branch = _classify_conditional_outcome(
                mismatch_position=mismatch_position,
                risk_positions=draft.risk_positions,
                risk_rank2_tokens=draft.risk_rank2_tokens,
                candidate_specs=candidate_specs,
                ar_tokens=ar_tokens,
            ...
            if selected_branch is not None:
                ...
                stats.prefetch_verified_hits_by_source[selected_branch] += 1
                if selected_branch == "continuation":
                    stats.continuation_verified_hits += 1
                    selected_prospective = continuation
                else:
                    selected_prospective = prospective_by_branch.get(selected_branch)
        ...
        if verified_hit and selected_prospective is not None:
            if forced_thinking_seed:
                ...
                stats.prefetch_hits_by_source[selected_branch] += 1
