#!/usr/bin/env python3
"""Merge fixed-margin-risk P1-conditional-new overlap stats into NeMo metrics."""
...
def summarize(rows: list[dict[str, Any]], wall_time_s: Optional[float]) -> dict[str, Any]:
    ...
    overlap_sum_fields = [
        ...
        "continuation_not_added_many_crossings", "prefetch_discarded_eos",
    ...
    for field, keys in (
        ...
        (
            "policy_case_rounds",
            ("c0_new", "c1_p1_new", "c2_p1_new", "c3plus_p1"),
        ),
        ("candidate_branch_executed", CANDIDATE_BRANCH_KEYS),
        ("prefetch_verified_hits_by_source", PREFETCH_SOURCE_KEYS),
        ("prefetch_hits_by_source", PREFETCH_SOURCE_KEYS),
    ):
        overlap[field] = {
            key: int(
                sum(
    ...
    decode_dense_slots = int(overlap["forward_distribution_decode"]["computed_token_sum"])
    decode_forward_count = int(overlap["forward_distribution_decode"]["count"])
    overlap["decode_dense_query_token_slots_total"] = decode_dense_slots
    overlap["decode_dense_query_token_slots_per_forward"] = rounded(
        decode_dense_slots / decode_forward_count if decode_forward_count else None
    )
    overlap["decode_dense_query_token_slots_per_output_token"] = rounded(
        decode_dense_slots / completion_tokens if completion_tokens else None
    )
    ...
    overlap["candidate_fixed_count"] = sum(
        int(outcome_states[state]["count"] or 0)
        for state in ("p1_rank2_fixed",)
    )
    overlap["candidate_wrong_count"] = sum(
        int(outcome_states[state]["count"] or 0)
        for state in ("p1_rank2_wrong",)
    )
    localized_states = (
        "p1_rank2_fixed", "p1_rank2_wrong",
        "p1_no_executable_candidate", "p2_omitted_rank2_would_fix",
        "p2_omitted_rank2_wrong", "p3_omitted_rank2_would_fix",
        "p3_omitted_rank2_wrong",
    )
    overlap["localized_error_count"] = sum(
        int(outcome_states[state]["count"] or 0) for state in localized_states
    )
    overlap["localized_error_rate"] = rounded(
        overlap["localized_error_count"] / attempts if attempts else None
    )
    correction: dict[str, dict[str, int | float | None]] = {}
    for branch in CANDIDATE_BRANCH_KEYS:
        checked = int(
            sum(
                as_float(
                    (((row.get("overlap") or {}).get("candidate_correction") or {}).get(branch) or {}).get("checked")
                )
                or 0
                for row in ok_rows
            )
        ...
        fixed = int(
            sum(
                as_float(
                    (((row.get("overlap") or {}).get("candidate_correction") or {}).get(branch) or {}).get("fixed")
                )
                or 0
                for row in ok_rows
            )
        ...
        wrong = int(
            sum(
                as_float(
                    (((row.get("overlap") or {}).get("candidate_correction") or {}).get(branch) or {}).get("wrong")
                )
                or 0
                for row in ok_rows
            )
        ...
        correction[branch] = {
            ...
            "fixed": fixed,
            "wrong": wrong,
            "fixed_rate": rounded(fixed / checked if checked else None),
            "wrong_rate": rounded(wrong / checked if checked else None),
        }
    overlap["candidate_correction"] = correction
...
def main() -> int:
    ...
    payload = {
        ...
        "metric_notes": {
            ...
            "outcome_states": "fourteen mutually-exclusive P1-conditional-new outcomes: locator misses, the executed P1 rank2 branch, omitted-P2/P3 counterfactual audits, and continuation states",
            "candidate_correction": "correctness when the verifier first rejects at the only executed correction branch P1",
            ...
            "decode_dense_query_token_slots_total": "sum of dense query-token slots over decode forwards only; each padded row slot is counted because it is physically computed",
            "decode_dense_query_token_slots_per_forward": "decode dense query-token slots divided by decode physical forwards",
            "decode_dense_query_token_slots_per_output_token": "decode dense query-token slots divided by returned completion tokens",
    ...
    metrics["pytorch_margin_risk_p1_conditional_new_overlap"] = payload
    ...
    summary_path = (
        ...
        else metrics_path.parent / "pytorch_margin_risk_p1_conditional_new_overlap_metrics_summary.json"
    )
