#!/bin/bash
# Isolated fixed-margin-risk P1-conditional-new overlap + NeMo-Skills evaluation.
...
DEFAULT_OUTPUT_PATH="/data/home/wly/dLLM/NLD_results/margin_risk_p1_conditional_new_overlap_results"
...
Modes:
  --mode overlap_base        Margin-risk P1-conditional-new overlap LinearSpec without LoRA
  --mode overlap_lora        Margin-risk P1-conditional-new overlap LinearSpec with segmented draft LoRA
...
Margin-risk P1-conditional-new overlap decoding:
...
Runtime/output:
  --efficiency-only         Ignore accuracy as the completion criterion; skip OOM requests (default)
  --require-accuracy        Restore strict behavior: any OOM/scoring failure fails the dataset
  ...
  --baseline-block16-dir DIR PyTorch+NeMo greedy block-16 baseline
  --baseline-block32-dir DIR PyTorch+NeMo greedy block-32 baseline
...
Example:
  bash $0 --mode overlap_lora --benchmarks gsm8k:1 --max-samples 1 --tokens 64 --gpu-device auto --block-length 16 --margin-risk-threshold 0.5
EOF
}
...
MODEL="$DEFAULT_MODEL"
SERVED_MODEL_NAME="nemotron-labs-diffusion-8b-margin-risk-p1-conditional-new-overlap"
TOKENS="8192"
TEMPERATURE="0"
TOP_P="0.95"
NUM_CHUNKS=""
...
BASELINE_BLOCK16_DIR="$DEFAULT_BASELINE_BLOCK16_DIR"
BASELINE_BLOCK32_DIR="$DEFAULT_BASELINE_BLOCK32_DIR"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --margin-risk-threshold) MARGIN_RISK_THRESHOLD="$2"; shift 2 ;;
        ...
        --baseline-block16-dir) BASELINE_BLOCK16_DIR="$2"; shift 2 ;;
        --baseline-block32-dir) BASELINE_BLOCK32_DIR="$2"; shift 2 ;;
        ...
        --require-accuracy) EFFICIENCY_ONLY="false"; shift ;;
...
case "$MODE" in
    overlap_base|margin_risk_p1_conditional_new_overlap_base) MODE="overlap_base" ;;
    overlap_lora|margin_risk_p1_conditional_new_overlap_lora) MODE="overlap_lora" ;;
...
is_positive_int "$BLOCK_LENGTH" && (( 10#$BLOCK_LENGTH >= 2 )) || { echo "ERROR: --block-length/--block-size must be an integer >= 2" >&2; exit 1; }
...
"$PYTORCH_PYTHON" - "$THRESHOLD" "$MARGIN_RISK_THRESHOLD" "$GPU_MIN_FREE_GB" <<'PY'
import math, sys
try:
    draft_threshold, margin_risk_threshold, min_free = map(float, sys.argv[1:])
except (TypeError, ValueError):
    raise SystemExit("--threshold, --margin-risk-threshold and --gpu-min-free-gb must be numeric")
if draft_threshold != 0.0:
    raise SystemExit("the P1-conditional-new overlap experiment requires --threshold 0")
...
if [[ "$EFFICIENCY_ONLY" != "true" ]] && evalplus_code_benchmark_requested && { [[ -n "$MAX_SAMPLES" ]] || [[ "$QUICK_TEST" == "true" ]]; }; then
...
if temperature != 0:
    raise SystemExit("the P1-conditional-new overlap experiment requires --temperature 0")
...
BASELINE_BLOCK16_DIR="$(realpath -m "$BASELINE_BLOCK16_DIR")"
...
RUN_NAME="margin_risk_p1_conditional_new_overlap_$(date +%Y%m%d_%H%M%S)"
...
echo "================================================================"
echo " Margin-risk-P1-conditional-new-overlap LinearSpec + NeMo-Skills eval"
echo "================================================================"
echo " Mode:                 $MODE"
echo " Model:                $MODEL"
...
echo " Margin-risk threshold:$MARGIN_RISK_THRESHOLD"
echo " Tokens/context:       $TOKENS / $CONTEXT_LENGTH"
echo " Client concurrency:   $CLIENT_CONCURRENCY (model execution serialized)"
echo " Efficiency only:      $EFFICIENCY_ONLY (OOM requests excluded from efficiency means)"
echo " Requested port:       $PORT (0 = atomic OS allocation)"
echo " Final output:         $FINAL_JOB_DIR"
echo " Baseline block 16:    $BASELINE_BLOCK16_DIR"
echo " Baseline block 32:    $BASELINE_BLOCK32_DIR"
...
payload = {
    ...
    "status": "initialized",
    "experiment": "fixed margin-risk P1 + conditional-new overlap: C0=new; C1=P1+new; C2=P1+new; C3+=P1",
    "entrypoint": "method/margin_risk_p1_conditional_new_overlap_linearspec/eval_margin_risk_p1_conditional_new_overlap.sh",
    "original_args": sys.argv[2:],
    "command": "bash method/margin_risk_p1_conditional_new_overlap_linearspec/eval_margin_risk_p1_conditional_new_overlap.sh " + " ".join(shlex.quote(x) for x in sys.argv[2:]),
    "backend": "native_pytorch_margin_risk_p1_conditional_new_overlap",
    "benchmark": {
        ...
        "efficiency_only": boolean("EFFICIENCY_ONLY"),
        "accuracy_required": not boolean("EFFICIENCY_ONLY"),
    ...
    "pytorch": {
        ...
        "max_total_rows": 3, "max_candidate_rows": 1, "max_speculative_rows": 2,
        "candidate_allocation_rule": "C0: continuation; C1: P1-rank2 plus continuation; C2: P1-rank2 plus continuation; C3+: P1-rank2; never execute P2/P3",
        ...
        "attention": "one causal verifier row plus up to two causal-prefix/bidirectional-suffix prospective rows, densely padded to common Q",
        "lora_routing": "segmented per token; verifier off, prospective suffix on",
        "alternative_token_rank_semantics": "rank2 among valid alternatives after excluding draft original token, MASK and EOS; the draft original token is treated as rank1; P2/P3 rank2 are computed only for counterfactual audit",
    ...
    "paths": {
        ...
        "baseline_block16_dir": env("BASELINE_BLOCK16"), "baseline_block32_dir": env("BASELINE_BLOCK32"),
    },
...
write_settings
"$PYTORCH_PYTHON" "$REPORT_SCRIPT" --result-dir "$FINAL_JOB_DIR" --baseline-block16-dir "$BASELINE_BLOCK16_DIR" --baseline-block32-dir "$BASELINE_BLOCK32_DIR"
...
export OVERLAP_MARGIN_RISK_THRESHOLD="$MARGIN_RISK_THRESHOLD"
...
export PYTORCH_EFFICIENCY_ONLY="$EFFICIENCY_ONLY"
...
export OVERLAP_BASELINE_BLOCK16_DIR="$BASELINE_BLOCK16_DIR"
export OVERLAP_BASELINE_BLOCK32_DIR="$BASELINE_BLOCK32_DIR"
...
export SEQ_EVAL_EXPNAME="margin_risk_p1_conditional_new_overlap"
...
if [[ "$PIPELINE_STATUS" != "0" ]]; then
    ...
    echo "ERROR: margin-risk-p1-conditional-new-overlap pipeline failed; internal work kept: $INTERNAL_JOB_DIR" >&2
...
"$PYTORCH_PYTHON" "$REPORT_SCRIPT" --result-dir "$FINAL_JOB_DIR" --baseline-block16-dir "$BASELINE_BLOCK16_DIR" --baseline-block32-dir "$BASELINE_BLOCK32_DIR" || true
echo "Completed margin-risk-p1-conditional-new-overlap LinearSpec + NeMo-Skills evaluation."
echo "Final output: $FINAL_JOB_DIR"
if [[ "$ANY_ERROR" != "0" ]]; then
