...
from method.margin_risk_p1_conditional_new_overlap_linearspec.hybrid import (
    build_multi_hybrid_attention_mask,
    repeat_dynamic_cache,
    select_and_crop_cache,
)
from method.margin_risk_p1_conditional_new_overlap_linearspec.segmented_lora import (
...
class CoreTests(unittest.TestCase):
    ...
    def test_analyze_keeps_first_three_crossings_and_counts_all(self) -> None:
        ...
        self.assertEqual([item.rank for item in state.candidates], [1])
        self.assertEqual([item.confidence_rank for item in state.candidates], [2])
        self.assertEqual([item.position for item in state.candidates], [1])
        self.assertEqual([item.branch_key for item in state.candidates], ["p1_rank2"])
        self.assertEqual(len(state.risk_rank2_tokens), 3)

    def test_two_crossings_execute_only_p1_but_keep_p2_audit_token(self) -> None:
        mask_id = 4
        logits = torch.tensor(
            [
        ...
        self.assertEqual(len(state.risk_rank2_tokens), 2)
        self.assertIsNotNone(state.risk_rank2_tokens[1])
    ...
    def test_executed_p1_and_omitted_p2_p3_outcomes(self) -> None:
        specs = (candidate(1, 2, 20, 2),)
        ar = torch.tensor([10, 20, 30, 40, 50, 60, 70])
        state, selected = _classify_conditional_outcome(
            mismatch_position=2,
            risk_positions=(2, 4, 6),
            risk_rank2_tokens=(20, 40, 60),
        ...
        state, selected = _classify_conditional_outcome(
            mismatch_position=2,
            risk_positions=(2, 4, 6),
            risk_rank2_tokens=(99, 99, 60),
            candidate_specs=wrong_specs,
            ar_tokens=ar,
            continuation=None,
        )
        self.assertEqual(state, "p1_rank2_wrong")
        ...
        self.assertEqual(state, "p2_omitted_rank2_would_fix")
        self.assertIsNone(selected)

        state, selected = _classify_conditional_outcome(
            mismatch_position=4,
            risk_positions=(2, 4, 6),
            risk_rank2_tokens=(20, 99, 60),
            candidate_specs=specs,
            ar_tokens=ar,
            continuation=None,
        )
        self.assertEqual(state, "p2_omitted_rank2_wrong")
        self.assertIsNone(selected)
        ...
        self.assertEqual(state, "p3_omitted_rank2_would_fix")
        self.assertIsNone(selected)

        state, selected = _classify_conditional_outcome(
            mismatch_position=6,
            risk_positions=(2, 4, 6),
            risk_rank2_tokens=(20, 40, 99),
            candidate_specs=specs,
            ar_tokens=ar,
            continuation=None,
        )
        self.assertEqual(state, "p3_omitted_rank2_wrong")
        self.assertIsNone(selected)

    def test_branch_checks_only_executed_p1(self) -> None:
        stats = OverlapGenerationStats()
        specs = (candidate(1, 2, 99, 2),)
    ...
    def test_forward_token_distribution_includes_dense_padding(self) -> None:
        ...
        stats.record_forward(
            ...
            rows=3,
            query_length=32,
            valid_lengths=(16, 21, 32),
        )
        payload = stats.to_dict()
        self.assertEqual(payload["processed_query_tokens"], 106)
        self.assertEqual(payload["valid_query_tokens"], 79)
        self.assertEqual(payload["padding_query_tokens"], 27)
        fused = payload["forward_kinds"]["multi_fused"]
        self.assertEqual(fused["computed_token_avg"], 96)
        self.assertEqual(fused["padding_token_sum"], 27)
        self.assertEqual(fused["computed_token_p50"], 96)

    def test_crossing_cases_lock_p1_conditional_new_row_policy(self) -> None:
        stats = OverlapGenerationStats()
        for count in (0, 1, 2, 3, 7):
            stats.record_crossing_count(count)
        self.assertEqual(
            stats.policy_case_rounds,
            {
                "c0_new": 1,
                "c1_p1_new": 1,
                "c2_p1_new": 1,
        ...
        # Intended fused row geometry including the verifier row.
        expected_rows = {0: 2, 1: 3, 2: 3, 3: 2}
        self.assertEqual(expected_rows, {0: 1 + 1, 1: 1 + 2, 2: 1 + 2, 3: 1 + 1})

    def test_transition_stats_do_not_impute_missing_next_round(self) -> None:
        stats = OverlapGenerationStats(prefetch_attempts=2)
        state = "p2_omitted_rank2_would_fix"
