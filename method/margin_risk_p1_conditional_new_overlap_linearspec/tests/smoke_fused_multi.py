#!/usr/bin/env python3
"""Real-model smoke for P1-conditional-new two/three-row fused paths."""
...
from method.margin_risk_p1_conditional_new_overlap_linearspec.hybrid import repeat_dynamic_cache
...
def forced_spec(
    ...
    original = int(draft.tokens[position].item())
    alternative = original
    for _ in range(confidence_rank - 1):
        alternative = choose_alternative(alternative, vocab_size, excluded + (original,))
...
def main() -> int:
    ...
    specs = (forced_spec(draft, 1, positions[0], vocab_size, excluded, 2),)
    forced = DraftState(
        tokens=draft.tokens,
        ...
        risk_rank2_tokens=(
            specs[0].alternative_token,
            choose_alternative(int(draft.tokens[positions[1]].item()), vocab_size, excluded),
            choose_alternative(int(draft.tokens[positions[2]].item()), vocab_size, excluded),
        ),
    )
    ...
    if not torch.equal(candidate_logits.argmax(dim=-1), expected):
        raise RuntimeError("two-row candidate verifier differs from normal causal verify")
    if candidate_new is not None or set(candidate_drafts) != {"p1_rank2"}:
        raise RuntimeError("candidate-only fused path returned incorrect branches")
    if candidate_stats.processed_rows != 2:
        raise RuntimeError("C3+ P1-conditional-new fused path must contain verifier+P1")
    for spec in specs:
        if int(candidate_drafts[spec.branch_key].tokens[0].item()) != spec.alternative_token:
            raise RuntimeError(f"candidate {spec.branch_key} prospective seed is not B")

    forced_c2 = DraftState(
        tokens=forced.tokens,
        confidences=forced.confidences,
        top1_top2_margins=forced.top1_top2_margins,
        candidates=specs,
        total_crossing_count=2,
        source="forced_c2_smoke",
        risk_positions=positions[:2],
        risk_rank2_tokens=forced.risk_rank2_tokens[:2],
    )
    ...
    if not torch.equal(continuation_logits.argmax(dim=-1), expected):
        raise RuntimeError("three-row continuation verifier differs from normal causal verify")
    if set(continuation_drafts) != {"p1_rank2"} or continuation_new is None:
        raise RuntimeError("candidate+continuation fused path returned incorrect branches")
    if continuation_stats.processed_rows != 3:
        raise RuntimeError("C2 fused path must contain verifier+P1+new")
