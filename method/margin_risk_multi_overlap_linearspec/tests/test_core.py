...
from method.margin_risk_multi_overlap_linearspec.hybrid import (
    build_multi_hybrid_attention_mask,
    repeat_dynamic_cache,
    select_and_crop_cache,
)
from method.margin_risk_multi_overlap_linearspec.segmented_lora import (
...
class CoreTests(unittest.TestCase):
    ...
    def test_analyze_keeps_first_three_crossings_and_counts_all(self) -> None:
        ...
        self.assertEqual([item.rank for item in state.candidates], [1, 2, 3])
        self.assertEqual([item.position for item in state.candidates], [1, 2, 3])
    ...
    def test_rank_specific_candidate_outcomes(self) -> None:
        specs = (candidate(1, 2, 20), candidate(2, 4, 40), candidate(3, 6, 60))
        ar = torch.tensor([10, 20, 30, 40, 50, 60, 70])
        for rank, position in ((1, 2), (2, 4), (3, 6)):
