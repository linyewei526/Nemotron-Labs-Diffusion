...
from method.margin_risk_multi_overlap_linearspec.merge_metrics import main, summarize
from method.margin_risk_multi_overlap_linearspec.report import (
    DEFAULT_BASELINE_16,
    DEFAULT_BASELINE_32,
    render,
)
from method.margin_risk_multi_overlap_linearspec.server import (
    ChatCompletionRequest,
    MarginRiskMultiOverlapEngine,
)
...
class MetricsAndReportTests(unittest.TestCase):
    ...
    def test_merge_can_create_efficiency_only_metrics_when_scorer_failed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            wrapper = payload["pytorch_margin_risk_multi_overlap"]
    ...
    def test_engine_turns_oom_into_failed_stat_and_empty_placeholder(self) -> None:
        engine = MarginRiskMultiOverlapEngine.__new__(MarginRiskMultiOverlapEngine)
        ...
        with (
            patch(
                "method.margin_risk_multi_overlap_linearspec.server.overlap_linear_spec_generate",
                side_effect=oom,
            ),
    ...
    def test_initial_report_exists_before_any_dataset_finishes(self) -> None:
        ...
        self.assertIn("P1、P2、P3 修正正确和错误", content)

    def test_report_renders_p1_p2_p3_and_forward_tokens(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            states["candidate_3_wrong"].update(outcome(2, 6, 1, 3, 4))
            states["candidate_3_wrong"].update(
                {
            ...
            payload = {
                ...
                "pytorch_margin_risk_multi_overlap": {
                    "decode": {
