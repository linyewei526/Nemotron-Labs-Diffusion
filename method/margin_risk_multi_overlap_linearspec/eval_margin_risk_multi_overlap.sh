...
DEFAULT_OUTPUT_PATH="/data/home/wly/dLLM/NLD_results/margin_risk_multi_overlap_results"
...
Modes:
  --mode overlap_base        Margin-risk-multi-overlap LinearSpec without LoRA
  --mode overlap_lora        Margin-risk-multi-overlap LinearSpec with segmented draft LoRA
...
Margin-risk-multi-overlap decoding:
  ...
  --margin-risk-threshold V  First three strict margin-risk crossings (default: 0.5)
...
MODEL="$DEFAULT_MODEL"
SERVED_MODEL_NAME="nemotron-labs-diffusion-8b-margin-risk-multi-overlap"
TOKENS="8192"
TEMPERATURE="0"
TOP_P="0.95"
NUM_CHUNKS=""
...
case "$MODE" in
    overlap_base|margin_risk_multi_overlap_base) MODE="overlap_base" ;;
    overlap_lora|margin_risk_multi_overlap_lora) MODE="overlap_lora" ;;
...
if draft_threshold != 0.0:
    raise SystemExit("the multi-overlap experiment requires --threshold 0")
...
if temperature != 0:
    raise SystemExit("the multi-overlap experiment requires --temperature 0")
...
RUN_NAME="margin_risk_multi_overlap_$(date +%Y%m%d_%H%M%S)"
...
echo "================================================================"
echo " Margin-risk-multi-overlap LinearSpec + NeMo-Skills eval"
echo "================================================================"
echo " Mode:                 $MODE"
echo " Model:                $MODEL"
...
payload = {
    ...
    "status": "initialized",
    "experiment": "fixed margin-risk first-three candidates plus full-pass continuation overlap",
    "entrypoint": "method/margin_risk_multi_overlap_linearspec/eval_margin_risk_multi_overlap.sh",
    "original_args": sys.argv[2:],
    "command": "bash method/margin_risk_multi_overlap_linearspec/eval_margin_risk_multi_overlap.sh " + " ".join(shlex.quote(x) for x in sys.argv[2:]),
    "backend": "native_pytorch_margin_risk_multi_overlap",
    "benchmark": {
...
export SEQ_EVAL_EXPNAME="margin_risk_multi_overlap"
...
if [[ "$PIPELINE_STATUS" != "0" ]]; then
    ...
    echo "ERROR: margin-risk-multi-overlap pipeline failed; internal work kept: $INTERNAL_JOB_DIR" >&2
...
"$PYTORCH_PYTHON" "$REPORT_SCRIPT" --result-dir "$FINAL_JOB_DIR" --baseline-block16-dir "$BASELINE_BLOCK16_DIR" --baseline-block32-dir "$BASELINE_BLOCK32_DIR" || true
echo "Completed margin-risk-multi-overlap LinearSpec + NeMo-Skills evaluation."
echo "Final output: $FINAL_JOB_DIR"
if [[ "$ANY_ERROR" != "0" ]]; then
