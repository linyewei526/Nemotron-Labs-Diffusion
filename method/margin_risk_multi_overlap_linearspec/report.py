#!/usr/bin/env python3
"""Incrementally render the fixed-margin-risk multi-overlap report."""
...
STATE_ZH = {
    ...
    "candidate_3_fixed": "P3修正对",
    "candidate_3_wrong": "P3修正错",
...
def decode(payload: dict[str, Any], *, new_method: bool) -> dict[str, Any]:
    key = "pytorch_margin_risk_multi_overlap" if new_method else "pytorch_native"
...
def render(result_dir: Path, baseline16: Path, baseline32: Path) -> str:
    ...
    lines = [
        "# 固定 margin-risk=0.5 多候选与 continuation overlap 实验报告",
        "",
        ...
        "## 1. 配置与 baseline",
        "",
        "变量说明：`BS` 是 block size；`MR阈` 是严格触发阈值；新方法最多使用 1 条 verifier row 加 3 条 speculative row。B16/B32 是既有 PyTorch+NeMo-Skills、LinearSpec LoRA、greedy、block size 16/32 baseline。示例：MR阈为 0.5 时，等于 0.5 不触发。",
        "",
        table(
    ...
    lines.extend([
        ...
        "## 2. 请求覆盖与解码效率",
        "",
        "变量说明：`Att/OK/Fail/OOM` 是请求尝试数、纳入效率聚合的成功数、排除的失败数和其中因显存不足跳过的数量；`Cov=OK/Att`。`TPF` 是成功请求返回 completion token/decode encoder forward，`NFE` 是成功请求每样本平均 decode forward，二者都排除 prompt prefill 并与 SGLang 对齐；`TPS` 是同步模型生成阶段 token/s。示例：Att=164、OK=163、OOM=1 时，所有效率均值只来自 163 个成功请求。四行 fused forward 仍只计一次，需结合 FwdTok 和 padding 表。accuracy 不进入本报告。",
        "",
    ])
    ...
    for dataset in completed:
        ...
        counts = {
            ...
            "p3f": integer(state_item(payload, "candidate_3_fixed").get("count")),
            "p3w": integer(state_item(payload, "candidate_3_wrong").get("count")),
    ...
    if completed:
        ...
        lines.append(table(
            ["数据集", "漏报", "预测后", "P1对", "P1错", "P2对", "P2错", "P3对", "P3错", "new命中", "new未用"],
            key_rows,
        ))
    else:
    ...
    lines.extend([
        ...
        "",
        "变量说明：下表保留全部 13 个互斥状态，并明确区分 P1、P2、P3 修正正确和错误。`Cnt` 是状态次数；`占比` 是 Cnt/融合轮；`本轮均` 是连续匹配 draft token 数加 verifier bonus 后的平均接收长度；`NextN/NextCov` 是确实存在同一 request 下一 verify 的数量/覆盖率；`配对本轮` 仅在这些配对样本上算当前均值；`下轮均` 是下一轮接收均值；`差值` 是逐对“下轮−当前轮”后平均。终止轮不补成 0。",
        "",
    ])
    ...
    lines.extend([
        ...
        "- 13 个细状态必须互斥且次数之和等于实际 fused overlap 轮数；报告读取 outcome_partition_valid 校验。",
