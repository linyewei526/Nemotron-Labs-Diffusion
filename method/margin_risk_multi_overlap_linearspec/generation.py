#!/usr/bin/env python3
"""Fixed-margin-risk multi-branch overlap LinearSpec generation.

Each fused call contains one padded causal verifier row and up to three
prospective rows.  Prospective rows are either corrections for the first three
strict margin-risk crossings or one full-block continuation.  Only the causal
verifier commits output and canonical KV state.
"""
    ...
    *,
    ...
    for position in range(1, tokens.shape[0]):
        ...
        if risk <= margin_risk_threshold:
            continue
        total_crossings += 1
        if total_crossings > MAX_CANDIDATES:
            continue
