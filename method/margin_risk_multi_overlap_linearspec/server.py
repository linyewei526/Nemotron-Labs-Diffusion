#!/usr/bin/env python3
"""Isolated OpenAI-compatible server for fixed-margin-risk multi-overlap."""
...
from method.margin_risk_multi_overlap_linearspec.generation import overlap_linear_spec_generate
...
class MarginRiskMultiOverlapEngine:
    ...
    def generate(
        ...
        if temperature != 0.0:
            raise ValueError("margin-risk-multi-overlap experiment currently requires temperature=0")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--served-model-name", default="nemotron-labs-diffusion-8b-margin-risk-multi-overlap")
...
def create_app(engine: MarginRiskMultiOverlapEngine) -> FastAPI:
    app = FastAPI(title="NLD Margin-Risk-Multi-Overlap LinearSpec Server", version="1.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
            ...
            "data": [
                {
                    ...
                    "object": "model",
                    "created": int(time.time()),
                    "owned_by": "local-margin-risk-multi-overlap",
                }
            ],
        }
    ...
    async def chat_completions(request: ChatCompletionRequest) -> dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Margin-risk-multi-overlap generation failed: {type(exc).__name__}: {exc}",
            ) from exc
        return {
            "id": request_id,
...
def main() -> int:
    args = parse_args()
    engine = MarginRiskMultiOverlapEngine(args)
