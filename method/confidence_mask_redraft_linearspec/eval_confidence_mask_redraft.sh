...
Modes:
  --mode mask_redraft_base   Autonomous all-MASK redraft without LoRA
  --mode mask_redraft_lora   Autonomous all-MASK redraft with segmented draft LoRA
...
Confidence MASK-redraft decoding:
...
Example:
  bash $0 --mode mask_redraft_lora --benchmarks gsm8k:1 --gpu-device auto --block-length 16 --drop-pct-threshold 0.15 --max-samples 2
EOF
}
...
MODEL="$DEFAULT_MODEL"
SERVED_MODEL_NAME="nemotron-labs-diffusion-8b-confidence-mask-redraft"
TOKENS="8192"
TEMPERATURE="0"
TOP_P="0.95"
NUM_CHUNKS=""
...
case "$MODE" in
    mask_redraft_base|confidence_mask_redraft_base) MODE="mask_redraft_base" ;;
    mask_redraft_lora|confidence_mask_redraft_lora) MODE="mask_redraft_lora" ;;
...
if [[ "$MODE" != "mask_redraft_lora" ]]; then
    LORA_PATH=""
fi
...
if draft_threshold != 0.0:
    raise SystemExit("the first MASK-redraft experiment requires --threshold 0")
...
if temperature != 0:
    raise SystemExit("the first MASK-redraft experiment requires --temperature 0")
...
if [[ "$MODE" == "mask_redraft_lora" && ! -f "$LORA_PATH/adapter_config.json" ]]; then
    echo "ERROR: mask_redraft_lora adapter_config.json not found: $LORA_PATH" >&2
    exit 1
fi
...
RUN_NAME="confidence_mask_redraft_linearspec_$(date +%Y%m%d_%H%M%S)"
...
echo "================================================================"
echo " Confidence MASK-redraft LinearSpec + NeMo-Skills eval"
echo "================================================================"
echo " Mode:                 $MODE"
echo " Model:                $MODEL"
...
payload = {
    ...
    "status": "initialized",
    "experiment": "confidence-triggered autonomous all-MASK redraft LinearSpec",
    "entrypoint": "method/confidence_mask_redraft_linearspec/eval_confidence_mask_redraft.sh",
    "original_args": sys.argv[2:],
    "command": "bash method/confidence_mask_redraft_linearspec/eval_confidence_mask_redraft.sh " + " ".join(shlex.quote(x) for x in sys.argv[2:]),
    ...
    "pytorch": {
        ...
        "autonomous_all_mask_redraft": True,
        "row_1_construction": "draft[:p] + MASK * block_length",
        "variable_length_reuse": True,
        "reuse_rule": "match every trustworthy verifier token from p through correction/bonus, then retain redraft[m-p:]",
...
export MASK_REDRAFT_DROP_PCT_THRESHOLD="$DROP_PCT_THRESHOLD"
...
export MASK_REDRAFT_SETTINGS_FILE="$FINAL_JOB_DIR/Settings.json"
...
export SEQ_EVAL_EXPNAME="confidence_mask_redraft"
...
if [[ "$PIPELINE_STATUS" != "0" ]]; then
    ...
    echo "ERROR: confidence MASK-redraft pipeline failed; internal work kept: $INTERNAL_JOB_DIR" >&2
...
echo "Completed confidence MASK-redraft LinearSpec + NeMo-Skills evaluation."
echo "Final output: $FINAL_JOB_DIR"
if [[ "$ANY_ERROR" != "0" ]]; then
