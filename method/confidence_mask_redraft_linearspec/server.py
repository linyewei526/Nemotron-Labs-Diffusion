#!/usr/bin/env python3
"""OpenAI-compatible server for confidence-triggered all-MASK redraft."""
...
from method.confidence_mask_redraft_linearspec.generation import (
    mask_redraft_linear_spec_generate,
)
...
class ConfidenceMaskRedraftEngine:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        if not torch.cuda.is_available():
            raise RuntimeError("CUDA is required for the 8B MASK-redraft experiment")
        ...
        if self.mode == "mask_redraft_lora":
            if not args.lora_path:
                raise RuntimeError("mask_redraft_lora requires --lora-path")
    ...
    def generate(
        ...
        if temperature != 0.0:
            raise ValueError("confidence-mask-redraft experiment currently requires temperature=0")
        ...
        with self._model_lock:
            ...
            try:
                ...
                with torch.inference_mode():
                    output_ids, decode_stats = mask_redraft_linear_spec_generate(
                        self.model,
                        prompt_ids,
                ...
                mask_redraft = decode_stats.to_dict()
                stat = {
                    "ok": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--served-model-name", default="nemotron-labs-diffusion-8b-mask-redraft"
    )
    parser.add_argument(
        "--mode", choices=["mask_redraft_base", "mask_redraft_lora"], required=True
    )
    parser.add_argument(
        "--dtype",
        default="bfloat16",
...
def create_app(engine: ConfidenceMaskRedraftEngine) -> FastAPI:
    app = FastAPI(title="NLD Confidence MASK-Redraft LinearSpec Server", version="1.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
            ...
            "data": [
                {
                    ...
                    "object": "model",
                    "created": int(time.time()),
                    "owned_by": "local-confidence-mask-redraft",
                }
            ],
        }
    ...
    async def chat_completions(request: ChatCompletionRequest) -> dict[str, Any]:
        ...
        request_id = f"chatcmpl-mask-redraft-{uuid.uuid4().hex}"
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Confidence MASK-redraft generation failed: {type(exc).__name__}: {exc}",
            ) from exc
        return {
            "id": request_id,
...
def main() -> int:
    args = parse_args()
    engine = ConfidenceMaskRedraftEngine(args)
