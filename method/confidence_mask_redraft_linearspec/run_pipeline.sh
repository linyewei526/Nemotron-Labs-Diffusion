#!/bin/bash
# Isolated confidence MASK-redraft PyTorch + NeMo-Skills pipeline.
...
PYTORCH_MODE="${PYTORCH_MODE:-mask_redraft_lora}"
PYTORCH_MODEL="${PYTORCH_MODEL:-/data1/linyewei/models/Nemotron-Labs-Diffusion-8B}"
PYTORCH_MODEL_TAG="${PYTORCH_MODEL_TAG:-nemotron-labs-diffusion-8b-mask-redraft}"
...
MASK_REDRAFT_DROP_PCT_THRESHOLD="${MASK_REDRAFT_DROP_PCT_THRESHOLD:-0.15}"
MASK_REDRAFT_SETTINGS_FILE="${MASK_REDRAFT_SETTINGS_FILE:-}"
...
SEQ_EVAL_EXPNAME="${SEQ_EVAL_EXPNAME:-confidence_mask_redraft}"
...
REQUEST_STATS_FILE="$PYTORCH_RUN_DIR/mask_redraft_request_stats.jsonl"
SERVER_LOG="$PYTORCH_RUN_DIR/mask_redraft_server.log"
...
wait_for_server_port() {
    ...
    for _ in $(seq 1 600); do
        if ! kill -0 "$pid" 2>/dev/null; then
            echo "ERROR: MASK-redraft server exited before publishing its port. Log tail:" >&2
            tail -120 "$log_file" >&2 || true
            return 1
        fi
        if [[ -s "$path" ]]; then
...
PY
    ...
    echo "ERROR: MASK-redraft server did not publish a port. Log tail:" >&2
    tail -120 "$log_file" >&2 || true
    return 1
}
...
echo "=============================================================="
echo " Confidence MASK-redraft LinearSpec + NeMo-Skills evaluation"
echo "=============================================================="
echo " Mode:               $PYTORCH_MODE"
echo " Drop-pct threshold: $MASK_REDRAFT_DROP_PCT_THRESHOLD"
...
SERVER_ARGS=(
    ...
    --drop-pct-threshold "$MASK_REDRAFT_DROP_PCT_THRESHOLD"
...
if [[ -n "$MASK_REDRAFT_SETTINGS_FILE" ]]; then
    "$PYTORCH_PYTHON" "$SETTINGS_UPDATER" "$MASK_REDRAFT_SETTINGS_FILE" --resolved-port "$PYTORCH_PORT" --status server_ready
fi
...
PY
    ...
    cp "$REQUEST_STATS_FILE" "$eval_dir/mask_redraft_request_stats.jsonl"
    set +e
...
echo "[5/5] Completed confidence MASK-redraft PyTorch evaluation pipeline."
