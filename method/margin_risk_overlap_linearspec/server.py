#!/usr/bin/env python3
"""Isolated OpenAI-compatible server for fixed-margin-risk overlap LinearSpec."""
...
from method.margin_risk_overlap_linearspec.generation import overlap_linear_spec_generate
...
class MarginRiskOverlapEngine:
    ...
    def generate(
        ...
        if temperature != 0.0:
            raise ValueError("margin-risk-overlap experiment currently requires temperature=0")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--served-model-name", default="nemotron-labs-diffusion-8b-margin-risk-overlap")
...
def create_app(engine: MarginRiskOverlapEngine) -> FastAPI:
    app = FastAPI(title="NLD Margin-Risk-Overlap LinearSpec Server", version="1.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
            ...
            "data": [
                {
                    ...
                    "object": "model",
                    "created": int(time.time()),
                    "owned_by": "local-margin-risk-overlap",
                }
            ],
        }
    ...
    async def chat_completions(request: ChatCompletionRequest) -> dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Margin-risk-overlap generation failed: {type(exc).__name__}: {exc}",
            ) from exc
        return {
            "id": request_id,
...
def main() -> int:
    args = parse_args()
    engine = MarginRiskOverlapEngine(args)
