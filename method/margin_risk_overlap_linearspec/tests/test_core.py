...
from method.margin_risk_overlap_linearspec.generation import (
    ...
    _analyze_draft,
    _classify_prefetch_outcome,
    _count_matched_draft_tokens,
)
from method.margin_risk_overlap_linearspec.hybrid import (
    build_hybrid_attention_mask,
    repeat_dynamic_cache,
    select_and_crop_cache,
)
from method.margin_risk_overlap_linearspec.segmented_lora import (
...
class CoreTests(unittest.TestCase):
    ...
    def test_hybrid_mask_supports_candidate_position_one(self) -> None:
        mask = build_hybrid_attention_mask(
            cache_length=3,
            block_length=4,
    ...
    def test_margin_risk_leftmost_crossing_and_excluded_alternative(self) -> None:
        mask_id = 4
        logits = torch.tensor(
            [
        ...
        self.assertIsNotNone(state.candidate_margin_risk)
        self.assertGreater(state.candidate_margin_risk, 0.5)

    def test_margin_risk_can_select_position_one_and_is_strict(self) -> None:
        ...
        self.assertEqual(crossing.candidate_position, 1)
        self.assertAlmostEqual(crossing.candidate_margin_risk, 1.0, places=6)
        equal_threshold = _analyze_draft(
            logits=logits,
            tokens=tokens,
            mask_token_id=2,
            excluded_alternative_token_ids=(2,),
            margin_risk_threshold=float(crossing.candidate_margin_risk),
            source="test",
        )
        self.assertIsNone(equal_threshold.candidate_position)

    def test_mask_is_excluded_from_margin_softmax(self) -> None:
        logits = torch.tensor([[4.0, 1.0, 100.0], [4.0, 1.0, 100.0]])
        state = _analyze_draft(
            logits=logits,
            tokens=torch.tensor([0, 0]),
            mask_token_id=2,
        ...
        expected_margin = float(torch.softmax(torch.tensor([4.0, 1.0]), dim=0).diff().abs().item())
        self.assertAlmostEqual(float(state.top1_top2_margins[1]), expected_margin, places=6)
        self.assertIsNone(state.candidate_position)

    def test_five_outcomes_are_mutually_exclusive(self) -> None:
        ar = torch.tensor([10, 20, 30, 40])
        cases = {
            "before_candidate_error": 1,
            "candidate_fixed_by_alternative": 2,
            "after_candidate_error": 3,
            "full_block_bonus": None,
        }
        for expected, mismatch in cases.items():
            actual = _classify_prefetch_outcome(
                mismatch_position=mismatch,
                candidate_position=2,
                alternative_token=20,
                ar_tokens=ar,
            )
            self.assertEqual(actual, expected)
        wrong = _classify_prefetch_outcome(
            mismatch_position=2,
            candidate_position=2,
            alternative_token=99,
            ar_tokens=ar,
        )
        self.assertEqual(wrong, "candidate_wrong_alternative")
        self.assertEqual(set(OUTCOME_STATES), set(cases) | {wrong})

    def test_transition_stats_do_not_impute_missing_next_round(self) -> None:
        stats = OverlapGenerationStats(prefetch_attempts=2)
        state = "candidate_fixed_by_alternative"
        ...
        self.assertEqual(payload["paired_current_accept_avg"], 3)
        self.assertEqual(payload["next_accept_avg"], 5)
