#!/usr/bin/env python3
"""OpenAI-compatible server for fixed-margin-risk P1/P2 plus always-New overlap."""
...
from method.margin_risk_two_plus_new_overlap_linearspec.generation import overlap_linear_spec_generate
from method.margin_risk_two_plus_new_overlap_linearspec.segmented_lora import install_segmented_lora
...
class MarginRiskTwoPlusNewOverlapEngine:
    ...
    def generate(
        ...
        if temperature != 0.0:
            raise ValueError("margin-risk-two-plus-new-overlap requires temperature=0")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--served-model-name", default="nemotron-labs-diffusion-8b-margin-risk-two-plus-new-overlap")
...
def create_app(engine: MarginRiskTwoPlusNewOverlapEngine) -> FastAPI:
    app = FastAPI(title="NLD Margin-Risk Two-Plus-New LinearSpec Server", version="1.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
    ...
    async def health() -> dict[str, Any]:
        return {
            ...
            "max_total_rows": 4,
            "max_risk_candidate_rows": 2,
            "always_new_when_boundary_allows": True,
            "max_prospective_rows": 3,
        }
            ...
            "data": [
                {
                    ...
                    "object": "model",
                    "created": int(time.time()),
                    "owned_by": "local-margin-risk-two-plus-new-overlap",
                }
            ],
        }
    ...
    async def chat_completions(request: ChatCompletionRequest) -> dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Margin-risk-two-plus-new-overlap generation failed: {type(exc).__name__}: {exc}",
            ) from exc
        return {
            "id": request_id,
...
def main() -> int:
    args = parse_args()
    engine = MarginRiskTwoPlusNewOverlapEngine(args)
