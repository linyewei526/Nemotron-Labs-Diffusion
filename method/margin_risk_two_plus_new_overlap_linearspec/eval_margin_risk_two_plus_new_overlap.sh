...
DEFAULT_OUTPUT_PATH="/data/home/wly/dLLM/NLD_results/margin_risk_two_plus_new_overlap_results"
...
Modes:
  --mode overlap_base        Margin-risk-two-plus-new-overlap LinearSpec without LoRA
  --mode overlap_lora        Margin-risk-two-plus-new-overlap LinearSpec with segmented draft LoRA
...
Margin-risk-two-plus-new-overlap decoding:
  ...
  --margin-risk-threshold V  First two strict margin-risk crossings (default: 0.5)
...
MODEL="$DEFAULT_MODEL"
SERVED_MODEL_NAME="nemotron-labs-diffusion-8b-margin-risk-two-plus-new-overlap"
TOKENS="8192"
TEMPERATURE="0"
TOP_P="0.95"
NUM_CHUNKS=""
...
case "$MODE" in
    overlap_base|margin_risk_two_plus_new_overlap_base) MODE="overlap_base" ;;
    overlap_lora|margin_risk_two_plus_new_overlap_lora) MODE="overlap_lora" ;;
...
if draft_threshold != 0.0:
    raise SystemExit("the two-plus-new overlap experiment requires --threshold 0")
...
if temperature != 0:
    raise SystemExit("the two-plus-new overlap experiment requires --temperature 0")
...
RUN_NAME="margin_risk_two_plus_new_overlap_$(date +%Y%m%d_%H%M%S)"
...
echo "================================================================"
echo " Margin-risk-two-plus-new-overlap LinearSpec + NeMo-Skills eval"
echo "================================================================"
echo " Mode:                 $MODE"
echo " Model:                $MODEL"
...
payload = {
    ...
    "status": "initialized",
    "experiment": "fixed margin-risk first-two candidates plus boundary-permitted always-New overlap",
    "entrypoint": "method/margin_risk_two_plus_new_overlap_linearspec/eval_margin_risk_two_plus_new_overlap.sh",
    "original_args": sys.argv[2:],
    "command": "bash method/margin_risk_two_plus_new_overlap_linearspec/eval_margin_risk_two_plus_new_overlap.sh " + " ".join(shlex.quote(x) for x in sys.argv[2:]),
    ...
    "pytorch": {
        ...
        "locator_rule": "first two left-to-right positions p in [1,L-1] with margin_risk > threshold; margin_risk=1-(P_top1-P_top2) after MASK exclusion",
        "max_total_rows": 4, "max_risk_candidate_rows": 2, "max_prospective_rows": 3,
        "continuation_rule": "for every round allowed by token/context/thinking boundaries, append current full draft plus one all-MASK block; reuse only when full verify bonus equals continuation token 0",
...
export SEQ_EVAL_EXPNAME="margin_risk_two_plus_new_overlap"
...
if [[ "$PIPELINE_STATUS" != "0" ]]; then
    ...
    echo "ERROR: margin-risk-two-plus-new-overlap pipeline failed; internal work kept: $INTERNAL_JOB_DIR" >&2
...
"$PYTORCH_PYTHON" "$REPORT_SCRIPT" --result-dir "$FINAL_JOB_DIR" --baseline-block16-dir "$BASELINE_BLOCK16_DIR" --baseline-block32-dir "$BASELINE_BLOCK32_DIR" || true
echo "Completed margin-risk-two-plus-new-overlap LinearSpec + NeMo-Skills evaluation."
echo "Final output: $FINAL_JOB_DIR"
if [[ "$ANY_ERROR" != "0" ]]; then
