...
from method.margin_risk_two_plus_new_overlap_linearspec.generation import (
    FORWARD_KINDS,
    OUTCOME_STATES,
)
from method.margin_risk_two_plus_new_overlap_linearspec.merge_metrics import main, summarize
from method.margin_risk_two_plus_new_overlap_linearspec.report import (
    DEFAULT_BASELINE_16,
    DEFAULT_BASELINE_32,
    decode,
    render,
)
from method.margin_risk_two_plus_new_overlap_linearspec.server import (
    ChatCompletionRequest,
    MarginRiskTwoPlusNewOverlapEngine,
)
...
class MetricsAndReportTests(unittest.TestCase):
    def test_report_converts_legacy_baseline_to_decode_only_tpf(self) -> None:
        payload = {
            "pytorch_native": {
                "decode": {
                    "request_count": 2,
                    ...
                    "forward_passes": 62,
                    "tokens_per_forward_pass": 246 / 62,
                }
            }
        }
        normalized = decode(payload, new_method=False)
        self.assertEqual(normalized["decode_forward_passes"], 60)
        self.assertEqual(normalized["average_forward_passes_per_sample"], 30)
        self.assertEqual(normalized["tokens_per_forward_pass"], 4.1)
        self.assertTrue(normalized["legacy_prefill_converted_for_report"])
    ...
    def test_merge_keeps_ranked_states_and_dense_forward_distribution(self) -> None:
        ...
        states["candidate_2_fixed"] = outcome(1, 5, 0, 0, 0)
        ...
        rows = [
            {
                ...
                "overlap": {
                    ...
                    "rounds_with_3plus_crossings": 1,
                    "risk_candidates_discarded_after_p2": 2,
                    "continuation_attempts_3plus_crossings": 1,
                    "continuation_verified_hits_3plus_crossings": 1,
                    "continuation_prefetch_hits_3plus_crossings": 1,
        ...
        p2 = overlap["outcome_states"]["candidate_2_fixed"]
        ...
        self.assertEqual(overlap["continuation_3plus_attempt_coverage"], 1)
        self.assertEqual(overlap["continuation_3plus_verified_hit_rate"], 1)
    ...
    def test_merge_can_create_efficiency_only_metrics_when_scorer_failed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            wrapper = payload["pytorch_margin_risk_two_plus_new_overlap"]
    ...
    def test_engine_turns_oom_into_failed_stat_and_empty_placeholder(self) -> None:
        engine = MarginRiskTwoPlusNewOverlapEngine.__new__(MarginRiskTwoPlusNewOverlapEngine)
        ...
        with (
            patch(
                "method.margin_risk_two_plus_new_overlap_linearspec.server.overlap_linear_spec_generate",
                side_effect=oom,
            ),
    ...
    def test_initial_report_exists_before_any_dataset_finishes(self) -> None:
        ...
        self.assertIn("P1、P2 修正正确和错误", content)
        self.assertIn("C3+ 轮次中用 New 替代 P3", content)

    def test_report_renders_p1_p2_always_new_and_forward_tokens(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            states["candidate_2_wrong"].update(outcome(2, 6, 1, 3, 4))
            states["candidate_2_wrong"].update(
                {
            ...
            payload = {
                ...
                "pytorch_margin_risk_two_plus_new_overlap": {
                    "decode": {
                        ...
                        "overlap": {
                            ...
                            "rounds_with_3plus_crossings": 2,
                            "risk_candidates_discarded_after_p2": 3,
                            "continuation_attempts_3plus_crossings": 2,
                            "continuation_3plus_attempt_coverage": 1,
                            "continuation_verified_hits_3plus_crossings": 1,
                            "continuation_3plus_verified_hit_rate": 0.5,
                            "continuation_prefetch_hits_3plus_crossings": 1,
                            "continuation_3plus_prefetch_hit_rate": 0.5,
            ...
            self.assertIn("P1对", content)
            self.assertIn("P2错", content)
            self.assertNotIn("P3错", content)
            self.assertIn("弃P3+", content)
