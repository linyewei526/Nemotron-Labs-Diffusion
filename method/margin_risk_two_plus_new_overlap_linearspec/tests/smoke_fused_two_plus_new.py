#!/usr/bin/env python3
"""Real-model smoke for the four-row Verify + P1 + P2 + New path."""
...
from method.margin_risk_two_plus_new_overlap_linearspec.hybrid import repeat_dynamic_cache
...
def main() -> int:
    ...
    positions = (1, max(2, args.block_size // 2))
    specs = tuple(
    ...
    forced = DraftState(
        ...
        candidates=specs,
        total_crossing_count=4,
        source="forced_two_plus_new_smoke",
    )
    ...
    for spec in specs:
        if int(continuation_drafts[spec.rank].tokens[0].item()) != spec.alternative_token:
