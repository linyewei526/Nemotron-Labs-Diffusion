...
from method.margin_risk_two_plus_new_overlap_linearspec.generation import (
    ...
    _analyze_draft,
    _classify_multi_outcome,
    _continuation_skip_reason,
    _count_matched_draft_tokens,
)
from method.margin_risk_two_plus_new_overlap_linearspec.hybrid import (
    build_multi_hybrid_attention_mask,
    repeat_dynamic_cache,
    select_and_crop_cache,
)
from method.margin_risk_two_plus_new_overlap_linearspec.segmented_lora import (
    SegmentedLoraController,
    SegmentedLoraLinear,
)


def candidate(rank: int, position: int, alternative: int) -> CandidateSpec:
    return CandidateSpec(rank, position, alternative, 0.8)
...
class CoreTests(unittest.TestCase):
    def test_new_policy_depends_only_on_sequence_boundaries(self) -> None:
        common = {
            "total_gen": 16,
            "block_length": 16,
            "max_new_tokens": 128,
        ...
        self.assertIsNone(_continuation_skip_reason(**common))
        self.assertEqual(
            _continuation_skip_reason(**{**common, "max_new_tokens": 32}),
            "no_future_round",
        )
        self.assertEqual(
            _continuation_skip_reason(**{**common, "max_positions": 63}),
            "context_limit",
        )
        self.assertEqual(
            _continuation_skip_reason(
    ...
    def test_analyze_keeps_first_two_crossings_and_counts_all(self) -> None:
        mask_id = 3
        logits = torch.tensor(
            [
                [6.0, 0.0, 0.0, 99.0],
        ...
        self.assertEqual([item.position for item in state.candidates], [1, 2])
    ...
    def test_rank_specific_candidate_outcomes(self) -> None:
        specs = (candidate(1, 2, 20), candidate(2, 4, 40))
        ar = torch.tensor([10, 20, 30, 40, 50, 60, 70])
        for rank, position in ((1, 2), (2, 4)):
            state, selected = _classify_multi_outcome(
                mismatch_position=position,
                candidate_specs=specs,
                ar_tokens=ar,
                continuation=None,
            )
            self.assertEqual(state, f"candidate_{rank}_fixed")
            self.assertEqual(selected, rank)
            wrong_specs = tuple(
                candidate(item.rank, item.position, 99 if item.rank == rank else item.alternative_token)
                for item in specs
            )
            state, selected = _classify_multi_outcome(
                mismatch_position=position,
            ...
            self.assertEqual(state, f"candidate_{rank}_wrong")
    ...
    def test_miss_and_full_outcomes_are_mutually_exclusive(self) -> None:
        ...
        for mismatch, expected in cases.items():
            ...
            self.assertIsNone(selected)
            observed.add(state)
        state, _ = _classify_multi_outcome(
            mismatch_position=2,
            candidate_specs=(),
            ar_tokens=ar,
            continuation=None,
        )
        self.assertEqual(state, "miss_no_candidate_error")
        observed.add(state)
        state, selected = _classify_multi_outcome(
        ...
        self.assertEqual((state, selected), ("full_continuation_hit", 0))
        observed.add(state)
        state, _ = _classify_multi_outcome(
            mismatch_position=None,
            candidate_specs=specs,
