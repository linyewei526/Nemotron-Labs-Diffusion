#!/bin/bash
# Isolated confidence strict direct MASK-redraft PyTorch + NeMo-Skills pipeline.
...
PYTORCH_MODE="${PYTORCH_MODE:-direct_mask_redraft_lora}"
PYTORCH_MODEL="${PYTORCH_MODEL:-/data1/linyewei/models/Nemotron-Labs-Diffusion-8B}"
PYTORCH_MODEL_TAG="${PYTORCH_MODEL_TAG:-nemotron-labs-diffusion-8b-direct-mask-redraft}"
...
DIRECT_MASK_REDRAFT_DROP_PCT_THRESHOLD="${DIRECT_MASK_REDRAFT_DROP_PCT_THRESHOLD:-0.15}"
DIRECT_MASK_REDRAFT_SETTINGS_FILE="${DIRECT_MASK_REDRAFT_SETTINGS_FILE:-}"
...
SEQ_EVAL_EXPNAME="${SEQ_EVAL_EXPNAME:-confidence_direct_mask_redraft}"
...
REQUEST_STATS_FILE="$PYTORCH_RUN_DIR/direct_mask_redraft_request_stats.jsonl"
SERVER_LOG="$PYTORCH_RUN_DIR/direct_mask_redraft_server.log"
...
wait_for_server_port() {
    ...
    for _ in $(seq 1 600); do
        if ! kill -0 "$pid" 2>/dev/null; then
            echo "ERROR: strict direct MASK-redraft server exited before publishing its port. Log tail:" >&2
            tail -120 "$log_file" >&2 || true
            return 1
        fi
        if [[ -s "$path" ]]; then
...
PY
    ...
    echo "ERROR: strict direct MASK-redraft server did not publish a port. Log tail:" >&2
    tail -120 "$log_file" >&2 || true
    return 1
}
...
echo "=============================================================="
echo " Confidence strict direct MASK-redraft LinearSpec + NeMo-Skills evaluation"
echo "=============================================================="
echo " Mode:               $PYTORCH_MODE"
echo " Drop-pct threshold: $DIRECT_MASK_REDRAFT_DROP_PCT_THRESHOLD"
...
SERVER_ARGS=(
    ...
    --drop-pct-threshold "$DIRECT_MASK_REDRAFT_DROP_PCT_THRESHOLD"
...
if [[ -n "$DIRECT_MASK_REDRAFT_SETTINGS_FILE" ]]; then
    "$PYTORCH_PYTHON" "$SETTINGS_UPDATER" "$DIRECT_MASK_REDRAFT_SETTINGS_FILE" --resolved-port "$PYTORCH_PORT" --status server_ready
fi
...
PY
    ...
    if [[ "$eval_status" != "0" ]]; then
        write_compact_error "$bench_name" benchmark_eval "$eval_status" "Benchmark evaluation failed" "$bench_log"
        PIPELINE_FAILED=1
        continue
    fi
    if [[ ! -f "$metrics_json" ]]; then
        write_compact_error "$bench_name" metrics_missing 1 "Benchmark runner did not produce metrics.json" "$bench_log"
        PIPELINE_FAILED=1
        continue
    fi

    cp "$REQUEST_STATS_FILE" "$eval_dir/direct_mask_redraft_request_stats.jsonl"
    set +e
    "$EVAL_PYTHON" "$MERGE_SCRIPT" --metrics-json "$metrics_json" --request-stats-file "$REQUEST_STATS_FILE" --benchmark "$bench_name" --wall-time-s "$wall_time" 2>&1 | tee -a "$bench_log"
...
echo "[5/5] Completed confidence strict direct MASK-redraft PyTorch evaluation pipeline."
