#!/usr/bin/env python3
"""OpenAI-compatible server for strict direct-hit all-MASK redraft."""
...
class ConfidenceDirectMaskRedraftEngine:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        if not torch.cuda.is_available():
            raise RuntimeError("CUDA is required for the 8B strict direct MASK-redraft experiment")
        ...
        if self.mode == "direct_mask_redraft_lora":
            if not args.lora_path:
                raise RuntimeError("direct_mask_redraft_lora requires --lora-path")
    ...
    def generate(
        ...
        if temperature != 0.0:
            raise ValueError("confidence-direct-mask-redraft requires temperature=0")
        ...
        with self._model_lock:
            ...
            try:
                ...
                with torch.inference_mode():
                    output_ids, decode_stats = direct_mask_redraft_linear_spec_generate(
                        self.model,
                        prompt_ids,
                ...
                direct_mask_redraft = decode_stats.to_dict()
                stat = {
                    "ok": True,
                    ...
                    "direct_mask_redraft": direct_mask_redraft,
                }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--served-model-name", default="nemotron-labs-diffusion-8b-direct-mask-redraft"
    )
    parser.add_argument(
        "--mode", choices=["direct_mask_redraft_base", "direct_mask_redraft_lora"], required=True
    )
    parser.add_argument(
        "--dtype",
        default="bfloat16",
...
def create_app(engine: ConfidenceDirectMaskRedraftEngine) -> FastAPI:
    app = FastAPI(title="NLD Confidence Strict Direct MASK-Redraft LinearSpec Server", version="1.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
            ...
            "data": [
                {
                    ...
                    "object": "model",
                    "created": int(time.time()),
                    "owned_by": "local-confidence-direct-mask-redraft",
                }
            ],
        }
    ...
    async def chat_completions(request: ChatCompletionRequest) -> dict[str, Any]:
        ...
        request_id = f"chatcmpl-direct-mask-redraft-{uuid.uuid4().hex}"
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Confidence strict direct MASK-redraft generation failed: {type(exc).__name__}: {exc}",
            ) from exc
        return {
            "id": request_id,
...
def main() -> int:
    args = parse_args()
    engine = ConfidenceDirectMaskRedraftEngine(args)
