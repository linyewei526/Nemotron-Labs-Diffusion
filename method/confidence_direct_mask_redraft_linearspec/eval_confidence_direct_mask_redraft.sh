#!/bin/bash
# Isolated confidence-triggered all-MASK redraft + NeMo-Skills evaluation.
...
REPORT_GENERATOR="$METHOD_DIR/report_results.py"
...
DEFAULT_BASELINE_B16="/data/home/wly/dLLM/NLD_results/observations/pytorch_nemo_eval_results/eval_20260804_120138"
DEFAULT_BASELINE_B32="/data/home/wly/dLLM/NLD_results/observations/pytorch_nemo_eval_results/eval_20260804_114935"
...
Modes:
  --mode direct_mask_redraft_base   Strict direct-hit all-MASK redraft without LoRA
  --mode direct_mask_redraft_lora   Strict direct-hit all-MASK redraft with segmented draft LoRA
...
Confidence strict direct MASK-redraft decoding:
...
Runtime/output:
  ...
  --baseline-b16 DIR         block_size=16 greedy baseline used by report.md
  --baseline-b32 DIR         block_size=32 greedy baseline used by report.md
...
Example:
  bash $0 --mode direct_mask_redraft_lora --benchmarks gsm8k:1 --gpu-device auto --block-length 16 --drop-pct-threshold 0.15 --max-samples 2
EOF
}
...
MODEL="$DEFAULT_MODEL"
SERVED_MODEL_NAME="nemotron-labs-diffusion-8b-direct-mask-redraft"
TOKENS="8192"
TEMPERATURE="0"
TOP_P="0.95"
NUM_CHUNKS=""
...
BASELINE_B16="$DEFAULT_BASELINE_B16"
BASELINE_B32="$DEFAULT_BASELINE_B32"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --baseline-b16) BASELINE_B16="$2"; shift 2 ;;
        --baseline-b32) BASELINE_B32="$2"; shift 2 ;;
...
case "$MODE" in
    direct_mask_redraft_base|confidence_direct_mask_redraft_base) MODE="direct_mask_redraft_base" ;;
    direct_mask_redraft_lora|confidence_direct_mask_redraft_lora) MODE="direct_mask_redraft_lora" ;;
...
if [[ "$MODE" != "direct_mask_redraft_lora" ]]; then
    LORA_PATH=""
fi
...
if draft_threshold != 0.0:
    raise SystemExit("the first strict direct MASK-redraft experiment requires --threshold 0")
...
if temperature != 0:
    raise SystemExit("the first strict direct MASK-redraft experiment requires --temperature 0")
...
if [[ "$MODE" == "direct_mask_redraft_lora" && ! -f "$LORA_PATH/adapter_config.json" ]]; then
    echo "ERROR: direct_mask_redraft_lora adapter_config.json not found: $LORA_PATH" >&2
    exit 1
fi
...
BASELINE_B16="$(realpath -m "$BASELINE_B16")"
BASELINE_B32="$(realpath -m "$BASELINE_B32")"
NEMO_SKILLS_DATA_DIR_ARG="$(realpath -m "$NEMO_SKILLS_DATA_DIR_ARG")"
GOOGLE_RESEARCH_DIR="$(realpath -m "$GOOGLE_RESEARCH_DIR")"
RUN_NAME="confidence_direct_mask_redraft_linearspec_$(date +%Y%m%d_%H%M%S)"
...
echo "================================================================"
echo " Confidence strict direct MASK-redraft LinearSpec + NeMo-Skills eval"
echo "================================================================"
echo " Mode:                 $MODE"
echo " Model:                $MODEL"
...
echo " Baseline B16/B32:     $BASELINE_B16 / $BASELINE_B32"
...
payload = {
    ...
    "status": "initialized",
    "experiment": "confidence-triggered strict direct-hit full-length all-MASK redraft LinearSpec",
    "entrypoint": "method/confidence_direct_mask_redraft_linearspec/eval_confidence_direct_mask_redraft.sh",
    "original_args": sys.argv[2:],
    "command": "bash method/confidence_direct_mask_redraft_linearspec/eval_confidence_direct_mask_redraft.sh " + " ".join(shlex.quote(x) for x in sys.argv[2:]),
    ...
    "pytorch": {
        ...
        "strict_direct_mask_redraft": True,
        "row_1_construction": "draft[:p] + MASK * block_length",
        "fixed_length_reuse": True,
        "reuse_rule": "reuse the full L-token row 1 iff m == p and R[0] == verifier correction C",
        "discard_rule": "discard row 1 for m < p, R[0] != C, m > p, and full-block bonus",
        "attention": "causal verifier row + causal-prefix/bidirectional all-MASK suffix row",
        "lora_routing": "segmented per token; verifier/prefix off, all-MASK redraft suffix on",
    ...
    "paths": {
        ...
        "baseline_b16": env("BASELINE_B16"), "baseline_b32": env("BASELINE_B32"),
...
export DIRECT_MASK_REDRAFT_DROP_PCT_THRESHOLD="$DROP_PCT_THRESHOLD"
...
export DIRECT_MASK_REDRAFT_SETTINGS_FILE="$FINAL_JOB_DIR/Settings.json"
...
export SEQ_EVAL_EXPNAME="confidence_direct_mask_redraft"
...
if [[ "$PIPELINE_STATUS" != "0" ]]; then
    ...
    echo "ERROR: confidence strict direct MASK-redraft pipeline failed; internal work kept: $INTERNAL_JOB_DIR" >&2
...
REPORT_STATUS=0
"$PYTORCH_PYTHON" "$REPORT_GENERATOR" \
    --result-dir "$FINAL_JOB_DIR" \
    --baseline-b16 "$BASELINE_B16" \
    --baseline-b32 "$BASELINE_B32" \
    --output "$FINAL_JOB_DIR/report.md" || REPORT_STATUS=$?
if [[ "$REPORT_STATUS" != "0" ]]; then
    echo "ERROR: failed to generate report.md" >&2
    ANY_ERROR=1
fi
...
echo "Completed confidence strict direct MASK-redraft LinearSpec + NeMo-Skills evaluation."
echo "Final output: $FINAL_JOB_DIR"
if [[ "$ANY_ERROR" != "0" ]]; then
